(function(){
  ...
  function heroHtml(data, source, announcements, index, total){
    ...
    return '<div class="mcj-hero-intro">' +
        '<h1>妙脆角电竞俱乐部</h1>' +
        '<p>专业陪玩｜极速上分｜快乐游戏</p>' +
        '<div class="mcj-hero-checks" aria-label="服务保障">' +
          '<span>✓ 极速接单</span><span>✓ 真人认证</span><span>✓ 24小时客服</span><span>✓ 安全保障</span>' +
        '</div>' +
        '<div class="mcj-hero-actions">' +
          '<a class="mcj-hero-btn" href="companion-center.html">立即找陪玩</a>' +
          '<a class="mcj-hero-btn" href="companion-apply.html">申请成为陪玩</a>' +
        '</div>' +
      '</div>' +
      '<div class="mcj-hero-banner-panel">' +
        '<a class="mcj-hero-image-link" href="' + safeAttr(data.link || '#') + '" target="' + safeAttr(data.linkTarget || '_self') + '" aria-label="' + safeAttr(data.name || '首页 Banner') + '">' +
  ...
  function ensureBelowHero(root, announcements){
    ...
    if (!notice) {
      ...
      notice.className = 'announcement-strip fade-cycle';
      root.insertAdjacentElement('afterend', notice);
    }
    notice.hidden = false;
    notice.innerHTML = '<b>📢 公告</b><span>欢迎来到 Meow Cui Jiao</span><a href="announcements.html">查看更多 &gt;</a>';
  }
