  ...
  return {
    ...
    payment: payment
      ? {
          ...
          bankAccountMasked: maskBankAccount(payment.payout_account_number || payment.bank_account),
          bankAccountFull: revealBank ? payment.payout_account_number || payment.bank_account || "" : "",
          hasBankAccount: !!(payment.payout_account_number || payment.bank_account),
          payoutBankName: payment.payout_bank_name || payment.bank_name || "",
          payoutAccountNumberMasked: maskBankAccount(payment.payout_account_number || payment.bank_account),
...
async function reviewMedia(req, companion, payload) {
  ...
  const reason = String(payload.rejectReason || payload.reason || "").trim();
  if ((status === "rejected" || status === "resubmit") && !reason) {
    throw Object.assign(new Error("驳回头像/相册/语音/视频时必须填写原因。"), { status: 400 });
  }
