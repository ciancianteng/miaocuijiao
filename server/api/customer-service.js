    ...
    if (action === "update_order_status") {
      ...
      try {
        ...
        if (transition.to === "cancelled" || transition.to === "refunded") {
          const clawMode = transition.to === "refunded" ? "refund" : "cancel";
          const clawOrder = patched || { ...order, status: transition.to };
          try {
          ...
          try {
            ...
            await clawbackBossCommissionForOrder(clawOrder, {
              reason: transition.to === "refunded" ? "订单退款扣回渠道佣金" : "订单取消扣回渠道佣金",
              mode: clawMode,
              refundAmount: body.refundAmount ?? body.refund_amount ?? null,
            });
          } catch (err) {
            console.warn("[cs] boss commission clawback", err?.message || err);
          }
          reward = await settleApi.clawbackCsOrderIncome(clawOrder, {
