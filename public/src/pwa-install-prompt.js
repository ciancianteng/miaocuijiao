(function () {
  ...
  var ICON_CACHE_V = "20260912pwaBrand8";
  var LOGO_FALLBACKS = [
    "/icons/icon-192.png",
  ...
  function logoCandidates() {
    ...
    // Keep logo URLs path-absolute (not origin-absolute).
    // Global avatar-fallback treats http://127.0.0.1/... as a bad URL and
    // would replace the brand mark with /default-avatar.png during local QA.
  ...
  function ensureDom() {
    ...
    rootEl.innerHTML =
      ...
      '<img class="mcj-pwa-logo" data-mcj-brand-logo="1" src="' +
      logoSrc() +
  ...
  function scheduleAutoShow() {
    // P1/P2: never auto-popup install sheet on normal browser visits.
    // Install guide opens only via MCJPwaInstall.openGuide() (user-initiated).
    return;
  ...
  function waitBrieflyForBipThenSchedule() {
    // Auto install prompts disabled — BIP still captured for manual install.
    return;
    var platform = detectPlatform();
