...
(function () {
  ...
  window.MCJ_DEFAULT_AVATAR = DEFAULT;
    ...
    if (img.classList && img.classList.contains("mcj-pwa-logo")) return true;
    if (typeof img.closest === "function" && img.closest(".mcj-header-brand, .mcj-pwa-root, .mcj-pwa-brand")) {
      return true;
    }
    var src = String(img.getAttribute("src") || img.src || "");
    if (/\/icons\/icon-\d+|apple-touch-icon|meow-cuijiao-brand/i.test(src)) return true;
    return false;
  }
