-- Gift / tipping system V1 + atomic accounting (PR179 / PR-B)
-- Filename: 08_* (do NOT use 06 — reserved for password-reset on main).
-- Safe to re-run. Does not DROP data.
--
-- APPLY ORDER (Production):
--   06_password_reset_requests.sql (if not applied)
--   07_orders_paid_at_paid_cat_food.sql (optional, unrelated)
--   08_gift_tipping_system_v1.sql  ← this file
--
-- ROLLBACK (soft): do not DROP tables/functions in prod.
--   1) Keep app fail-closed without mcj_send_gift_tip (gifts return 503).
--   2) Optionally: revoke execute on mcj_send_gift_tip from service_role.
--   Hard DROP of tables/functions is destructive and not recommended.
--
-- Atomic guarantee: mcj_send_gift_tip runs wallet debit + gift_transactions
-- + companion_income inside one PostgreSQL function/transaction.

-- 0) Greenfield base tables (Production currently missing gifts*)
