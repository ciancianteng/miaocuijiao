(function () {
  ...
  function bindReviewExpand(root) {
    ...
    root.querySelectorAll(".pd-review-item").forEach(function (card) {
      ...
      requestAnimationFrame(function () {
        var needs = body.scrollHeight > body.clientHeight + 2;
        btn.hidden = !needs;
      });
    });
  }
  ...
  function render(c) {
    ...
    if (!galleryUrls.length && image) galleryUrls = [image];
    var galleryWall =
      galleryList.length > 0
        ? galleryList
            ...
            .join("")
        : '<img class="mcj-album-thumb" data-album-index="0" src="' +
          esc(image) +
          '" alt="' +
          esc(c.name) +
          ' 卡面" onerror="this.onerror=null;this.src=\'/default-avatar.png\'">';
    ...
    s.innerHTML =
      ...
      voice +
      '</div></div></section><section class="detail-card game-wall"><div class="section-head"><h2>' +
      (galleryList.length ? "相册" : "卡面图片") +
      '</h2></div><div class="wall-grid" data-profile-album>' +
      galleryWall +
      '</div></section><section class="detail-card real-review-wall"><div class="section-head"><h2>真实订单评价</h2><span>好评 ' +
      esc(goodText) +
      " · 共 " +
      esc(c.reviewCount || 0) +
