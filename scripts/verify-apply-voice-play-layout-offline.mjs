#!/usr/bin/env node
/**
 * Offline: voice play CTA layout + play state labels + ObjectURL lifecycle order.
 */
...
const checks = [
  ...
  ["no render on ended listen", !/ended[\s\S]{0,400}writeDraftRecord\(draft\);\s*render\(/.test(js)],
  ["revoke after refreshVoiceUi", /refreshVoiceUi\(\);\s*liveVoiceBlob = null;\s*if \(liveVoiceObjectUrl\)/.test(js)],
  ["prefer liveVoiceObjectUrl on play", /playSrc\s*=\s*\n?\s*liveVoiceObjectUrl/.test(js) || /var playSrc =\s*[\s\S]{0,80}liveVoiceObjectUrl/.test(js)],
  ["player not 40px grid", !/voice-card-player\{[^}]*grid-template-columns:\s*40px/.test(css.replace(/\s+/g, " "))],
  ["play-cta nowrap", /voice-card-play-cta[\s\S]{0,400}white-space:\s*nowrap/.test(css)],
  ["play-cta row", /voice-card-play-cta[\s\S]{0,200}flex-direction:\s*row/.test(css)],
...
if (!exe) {
  console.log("SKIP layout-browser (no chrome/edge)");
...
await page.locator("#applyVoicePanel").screenshot({
  path: "artifacts/apply-voice-confirm/02-pending-confirm.png",
});
await browser.close();
...
const layoutOk =
  ...
  metrics.flexDir === "row" &&
  /horizontal-tb/i.test(metrics.writingMode || "horizontal-tb") &&
  metrics.text === "▶播放试听";
