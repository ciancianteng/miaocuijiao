/**
 * Runtime guard: refuse destructive/seed scripts against known Production Supabase projects.
 *
...
export function assertNonProductionSupabase(scriptName = "script", url = process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL || "") {
  ...
  if (allow) {
    console.warn(`[prod-guard] ALLOWED write to production Supabase by explicit override (${scriptName}) ref=${supabaseProjectRef(url)}`);
