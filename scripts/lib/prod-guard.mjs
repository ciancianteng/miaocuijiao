...
const PRODUCTION_HOST_RE =
  /(?:^|\.)meowcuijiao\.com$/i;
const STAGING_HOST_HINT_RE =
  /staging|vercel\.app|localhost|127\.0\.0\.1/i;
...
export function isProductionAppBase(base = "") {
  ...
  return PRODUCTION_HOST_RE.test(host) || host === "www.meowcuijiao.com" || host === "meowcuijiao.com";
}
 ...
 * Blocks:
 * - Production app hosts (meowcuijiao.com)
 * - Production Supabase project ref
 * - DATABASE_URL / POSTGRES URLs that contain the Production ref
 *
 * @param {{ script?: string, base?: string, supabaseUrl?: string, databaseUrl?: string, requireStagingSupabase?: boolean }} opts
 */
export function assertSmokeTargetAllowed(opts = {}) {
  const script = opts.script || opts.name || "smoke";
  const base = opts.base || process.env.BASE || process.env.MCJ_STAGING_URL || process.env.MCJ_PREVIEW_URL || "";
  const supabaseUrl =
    opts.supabaseUrl ||
  ...
  if (supabaseUrl) {
    ...
    if (opts.requireStagingSupabase && !isKnownStagingSupabase(supabaseUrl) && !prodWriteOverrideAllowed()) {
      throw new Error(
  ...
  if (dbBlob && dbBlob.toLowerCase().includes(PRODUCTION_SUPABASE_REF) && !prodWriteOverrideAllowed()) {
    throw new Error(
  ...
  if (base && !isStagingOrPreviewAppBase(base) && !prodWriteOverrideAllowed()) {
    // Unknown hosts are refused for smoke writes unless clearly staging/preview/local.
    throw new Error(
