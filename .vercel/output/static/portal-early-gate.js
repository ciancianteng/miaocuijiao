...
(function () {
  ...
  var GATE_VERSION = "20260804authP0hard";
  ...
  function evaluate() {
    ...
    if (/\/(mine|orders|support|recharge|messages|favorites|profile)\.html$/i.test(p)) {
      hideShell();
      ...
      var bossRoleOk =
        isBossRole(roleOf(bossUser)) ||
        isBossRole(bossShared) ||
        (!bossShared && !roleOf(bossUser));
      // Non-boss roles must never open boss center via shared JWT.
      if (bossShared && !isBossRole(bossShared)) {
