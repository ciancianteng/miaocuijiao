(function(){
  ...
  function hasAdminPermissionFor(required){var role=adminPermissionRole();if(!role)return false;if(required==='admin')return role==='admin'||role==='super_admin'||role==='finance_admin';if(required==='super_admin')return role==='super_admin'||role==='admin';return role===required||role==='super_admin'}
  ...
  function hasAdminLiveAuth(){
    var access=authAccessToken();
    var refresh=readStorageItem('mcjAuthRefreshToken');
  ...
  function enforceRole(){
    ...
    if(adminOnly){
      ...
      var token=readStorageItem('adminAuthToken');
      var logged=gate?gate.isLogged('admin'):(String(token).indexOf('admin_session_')===0&&hasAdminLiveAuth());
      // Soft token alone NEVER unlocks — require JWT/refresh.
      if(!token||!logged||!hasAdminLiveAuth()){
  ...
  function statusChip(text){var t=String(text||'');var cls=/通过|完成|成功|在线|正常|开启|显示|接待中/.test(t)?'ok':/拒绝|冻结|异常|取消|离线|关闭|隐藏|停用/.test(t)?'bad':'wait';return '<span class="chip '+cls+'">'+esc(t)+'</span>'}
  function esc(s){return String(s==null?'':s).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}
  /** Hide DB UUID / auth.uid / internal ids from ops-facing admin lists. */
  function isDbUuid(v){return /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(v||'').trim())}
  ...
  function preferPublicCode(candidates,fallback){
    var list=Array.isArray(candidates)?candidates:[candidates];
    for(var i=0;i<list.length;i++){
      var v=String(list[i]==null?'':list[i]).trim();
      if(!v||v==='-'||v==='未设置'||isDbUuid(v)||isDevLogin(v))continue;
      return v;
    }
    return fallback||'未分配';
  }
  function adminDevInfoHtml(){
    // Formal admin UI: never show UUID / .meow.test / Internal ID blocks.
    return '';
  }
  window.MCJAdminPrivacy={isDbUuid:isDbUuid,isDevLogin:isDevLogin,publicEmailLabel:publicEmailLabel,preferPublicCode:preferPublicCode,adminDevInfoHtml:adminDevInfoHtml};
  function table(headers,rows){var bodyHtml='';if(Array.isArray(rows)){bodyHtml=rows.length?rows.join(''):'';}else if(typeof rows==='string'){bodyHtml=rows;}var emptyRow='<tr><td colspan="'+headers.length+'"><div class="empty">暂无数据</div></td></tr>';return '<div class="table-wrap"><table><thead><tr>'+headers.map(function(h){return '<th>'+h+'</th>'}).join('')+'</tr></thead><tbody>'+(bodyHtml||emptyRow)+'</tbody></table></div>'}
  ...
  var playerMorePopover={el:null,anchor:null,playerId:'',onScroll:null};
  function closePlayerMoreMenu(){
    if(playerMorePopover.el){
      playerMorePopover.el.remove();
      playerMorePopover.el=null;
    }
    playerMorePopover.anchor=null;
    playerMorePopover.playerId='';
    if(playerMorePopover.onScroll){
      document.querySelectorAll('#playerManagement .player-table-wrap,.player-table-wrap').forEach(function(scroller){
  ...
  function openPlayerMoreMenu(anchorBtn){
    var playerId=String(anchorBtn.getAttribute('data-player-id')||'').trim();
    if(!playerId)return;
    if(playerMorePopover.el&&playerMorePopover.playerId===playerId){closePlayerMoreMenu();return;}
    closePlayerMoreMenu();
    closeBossMoreMenu();
    var items=[
      ['view','income','查看流水'],
    ...
    menu.className='player-more-popover';
    menu.setAttribute('role','menu');
    menu.setAttribute('data-player-more-popover','1');
    menu.innerHTML=items.map(function(item){
      var extra=item[1]?' data-player-section="'+esc(item[1])+'"':'';
      var danger=/freeze|disable/.test(item[0])?' class="danger-text"':'';
      return '<button type="button" role="menuitem"'+danger+' data-player-action="'+esc(item[0])+'" data-player-id="'+esc(playerId)+'"'+extra+'>'+esc(item[2])+'</button>';
    }).join('');
    document.body.appendChild(menu);
    playerMorePopover.el=menu;
    playerMorePopover.anchor=anchorBtn;
    playerMorePopover.playerId=playerId;
  ...
  var adminModalOverlayBridge=false;
  var dashboardState={pendingItems:[],pendingPage:1,pendingPageSize:8,totalPending:0,orderSeries:[],revenueSeries:[],profitSeries:[]};
  function extractModalTitle(html,fallback){
    if(!html)return fallback||'详情';
    var tmp=document.createElement('div');
    tmp.innerHTML=html;
    var heading=tmp.querySelector('h2,h3');
    return heading&&heading.textContent?(heading.textContent||'').trim():(fallback||'详情');
  }
  function restoreModalBodyAlias(){
    var overlayBody=document.querySelector('#adminOverlay [data-admin-overlay-body]');
    if(overlayBody&&overlayBody.id==='modalBody')overlayBody.id='';
    var legacy=document.getElementById('modalBodyLegacy');
    if(legacy){legacy.id='modalBody';legacy.innerHTML='';}
    adminModalOverlayBridge=false;
  }
  function closeAdminOverlay(){
    if(window.MCJAdminOverlay&&window.MCJAdminOverlay.isOpen()){
      if(adminModalOverlayBridge)restoreModalBodyAlias();
      window.MCJAdminOverlay.close();
    }
  }
  function closeAdminModal(){
    closeBossMoreMenu();
    if(window.MCJAdminOverlay&&adminModalOverlayBridge){
      window.MCJAdminOverlay.close();
    }else{
      restoreModalBodyAlias();
    }
  ...
  function openAdminModal(title){
    var legacy=document.getElementById('modalBody');
    if(window.MCJAdminOverlay&&legacy){
      var html=legacy.innerHTML;
      var overlayBody=window.MCJAdminOverlay.open({
        title:title||extractModalTitle(html,'详情'),
        html:html,
        onClose:function(){restoreModalBodyAlias();bossAdminState.activeBossId='';}
      });
      legacy.id='modalBodyLegacy';
      legacy.innerHTML='';
      if(overlayBody)overlayBody.id='modalBody';
      adminModalOverlayBridge=true;
      return;
    }
    var modal=document.getElementById('adminModal');
    if(!modal)return;
  ...
  function bossCountryFilterOptionsHtml(){
    var html='<option value="">国家/地区</option>';
    if(window.MCJCountries&&Array.isArray(window.MCJCountries.list)){
      window.MCJCountries.list.forEach(function(c){
        html+='<option value="'+esc(c.code)+'">'+esc((c.flag||'')+' '+(c.nameZh||c.name||c.code))+'</option>';
      });
      return html;
    }
    [['MY','🇲🇾 马来西亚'],['CN','🇨🇳 中国'],['SG','🇸🇬 新加坡'],['TW','🇹🇼 台湾'],['HK','🇭🇰 香港']].forEach(function(pair){
  ...
  function renderBossManagement(){
    ...
    target.innerHTML='<div class="boss-admin-page compact">'+
      ...
      '<div class="boss-filter-bar" data-boss-filter-bar>'+
        ...
        '<select class="boss-filter-field" data-boss-filter="country" aria-label="国家/地区">'+bossCountryFilterOptionsHtml()+'</select>'+
        '<input class="boss-filter-field" type="date" data-boss-filter="registered" aria-label="注册日期">'+
        '<input class="boss-filter-field boss-filter-search" data-boss-search placeholder="老板编号 / 昵称 / 手机号 / 邮箱" aria-label="搜索">'+
  ...
  function normalizeBossAdmin(boss){
    ...
    var bossUidRaw=bossValue(boss,['boss_uid','bossUid','publicUid','public_uid'],'');
    var uid=preferPublicCode([bossUidRaw,bossValue(boss,['uid','systemUid','system_uid'],''),bossValue(boss,['bossId','boss_id','publicId','public_id'],'')],'未分配');
    var bossId=uid;
    ...
    var countryCode=String(bossValue(boss,['countryCode','country_code'],'MY')||'MY').toUpperCase();
    var countryLabel=bossValue(boss,['countryLabel','country_label'],'');
    if(!countryLabel&&window.MCJCountries&&typeof window.MCJCountries.labelOf==='function')countryLabel=window.MCJCountries.labelOf(countryCode);
    if(!countryLabel)countryLabel=countryCode;
    var phoneE164=bossValue(boss,['phoneE164','phone_e164'],'');
    var phoneDisplay=bossValue(boss,['phoneDisplay','phone_display'],'')||phoneE164||bossValue(boss,['phone','mobile'],'-');
    var rawEmail=bossValue(boss,['email'],'');
    var email=publicEmailLabel(rawEmail);
    var search=[uid,bossId,internalId,name,boss.phone,phoneDisplay,phoneE164,rawEmail,status,boss.vip,boss.vipLevel,countryCode,countryLabel].join(' ').toLowerCase();
    return {raw:boss,id:internalId||uid,internalId:internalId||'',uid:uid,bossUid:uid,bossId:bossId,name:name,phone:bossValue(boss,['phone','mobile'],'-'),phoneDisplay:phoneDisplay,phoneE164:phoneE164,countryCode:countryCode,countryLabel:countryLabel,email:email,rawEmail:rawEmail,avatar:boss.avatar||boss.avatar_url||'assets/meow-cuijiao-brand.jpg',vip:bossValue(boss,['vip','vipLevel'],'VIP0'),balance:bossValue(boss,['balance','walletBalance','wallet_balance'],'0猫粮'),paidBalance:bossValue(boss,['paidBalance','paid_balance'],'0'),bonusBalance:bossValue(boss,['bonusBalance','bonus_balance'],'0'),totalRecharge:bossValue(boss,['totalRecharge','total_recharge','rechargeTotal'],'RM0'),totalSpent:bossValue(boss,['total_spent','totalSpent','totalConsume','total_consume'],'0猫粮'),totalCompensation:bossValue(boss,['totalCompensation','total_compensation'],'0'),totalOrders:bossValue(boss,['totalOrders','total_orders','orderCount'],'0'),refundAmount:bossValue(boss,['refundAmount','refund_amount','totalRefund'],'RM0'),registered:registered,lastLogin:lastLogin,status:status,remark:bossValue(boss,['remark','note','adminRemark'],''),search:search};
  }
  function visibleBossRows(){
    ...
    return (bossAdminState.rows||[]).map(normalizeBossAdmin).filter(function(item){
      ...
      if(ok&&filters.country)ok=String(item.countryCode||'').toUpperCase()===String(filters.country).toUpperCase();
  ...
  function renderBossTableRows(){
    ...
    var headers=['老板编号','头像','昵称','VIP','余额','累计充值','累计消费','订单数量','状态','操作'];
    var body=pageRows.map(function(item){return '<tr class="boss-list-row" data-boss-open="'+esc(item.id)+'">'+
      '<td data-label="老板编号" title="'+esc(item.uid)+'"><button class="boss-id-link" type="button" data-boss-action="view" data-boss-id="'+esc(item.id)+'">'+esc(item.uid)+'</button></td>'+
      ...
      '<td data-label="操作" class="boss-action-cell"><button class="mini-btn" type="button" data-boss-action="view" data-boss-id="'+esc(item.id)+'">查看</button>'+adminDevInfoHtml([['Internal ID',item.internalId],['Auth UID',item.internalId],['开发邮箱',item.rawEmail&&isDevLogin(item.rawEmail)?item.rawEmail:'']])+'</td>'+
    '</tr>';}).join('');
  ...
  function exportBossRows(rows){
    ...
    var headers=['老板编号','昵称','VIP','余额','累计充值','累计消费','订单数量','状态'];
      ...
      '<div class="boss-detail-panels">'+
        '<section class="boss-detail-panel'+(activeTab==='profile'?' active':'')+'" data-boss-panel="profile">'+
          '<div class="boss-detail-grid"><section><h3>基本资料</h3><div class="detail-list" data-boss-profile-list>'+
            ...
            '<div><span>老板编号</span><strong>'+esc(item.uid)+'</strong></div>'+
            adminDevInfoHtml([['Internal ID',item.internalId],['Auth UID',item.internalId],['开发邮箱',item.rawEmail&&isDevLogin(item.rawEmail)?item.rawEmail:'']])+
            '<div><span>国家 / 地区</span><strong>'+esc(item.countryLabel||item.countryCode||'MY')+'</strong></div>'+
            '<div><span>国际手机号</span><strong>'+esc(item.phoneDisplay||item.phoneE164||item.phone||'-')+'</strong></div>'+
  ...
  function normalizeOrder(order){
    ...
    var bossPublic=preferPublicCode([orderValue(order,['bossUid','boss_uid','customerUid','customer_uid'],''),orderValue(order,['bossId','boss_id'],'')],'');
    var playerPublic=preferPublicCode([orderValue(order,['playerUid','player_uid','companionCode','companion_code','publicId'],''),orderValue(order,['companionUid','companion_uid'],'')],'');
    var orderNo=preferPublicCode([orderValue(order,['orderNo','order_no'],'')],'')||orderValue(order,['orderNo','order_no','id'],'-');
    if(isDbUuid(orderNo))orderNo=preferPublicCode([orderValue(order,['orderNo','order_no'],'')],'—');
    return Object.assign({},order,{
      id:orderNo,
      rawId:orderValue(order,['id','uuid'],''),
  ...
  function orderManagementHtml(orders,summary,configured){
    ...
    var rows=orders.map(function(o){return '<tr data-order-row data-order-status="'+esc(o.orderStatus)+'" data-order-type="'+esc(o.type)+'" data-payment-status="'+esc(o.paymentStatus)+'" data-game="'+esc(o.game)+'" data-service="'+esc(o.serviceStaff)+'" data-player="'+esc(o.playerName+' '+o.playerUid)+'" data-source="'+esc(o.source)+'" data-amount="'+esc(o.amount)+'" data-created-at="'+esc(o.createdAt)+'" data-search="'+esc([o.id,o.type,o.bossName,o.bossUid,o.bossId,o.playerName,o.playerUid,o.serviceStaff,o.game,o.paymentOrderNo].join(' '))+'">'+
      '<td><strong>'+esc(o.id)+'</strong></td><td>'+orderStatusChip(o.type)+'</td><td>'+esc(o.bossName)+'</td><td>'+esc(o.bossUid)+'</td><td>'+esc(o.playerName)+'</td><td>'+esc(o.playerUid)+'</td><td>'+esc(o.game)+'</td><td>'+esc(o.serviceContent)+'</td><td>'+esc(o.serviceStaff)+'</td><td>'+moneyText(o.amount)+'</td><td>'+moneyText(o.playerIncome)+'</td><td>'+moneyText(o.platformProfit)+'</td><td>'+orderStatusChip(o.paymentStatus)+'</td><td>'+orderStatusChip(o.orderStatus)+'</td><td>'+esc(o.createdAt)+'</td><td>'+esc(o.serviceTime)+'</td><td><div class="order-row-actions">'+orderActions(o).map(function(a){return '<button class="mini-btn '+(a.danger?'danger-btn':'')+'" data-order-action="'+esc(a.key)+'" data-order-id="'+esc(o.rawId||o.id)+'" type="button">'+esc(a.label)+'</button>'}).join('')+'</div>'+adminDevInfoHtml([['Order UUID',o.rawId&&isDbUuid(o.rawId)?o.rawId:''],['Boss Internal',orderValue(o,['boss_id','bossId'],'')],['Companion Internal',orderValue(o,['companion_id','companionId'],'')]])+'</td></tr>'}).join('');
  ...
  function renderOrderDetail(order){
    ...
    var detail=[
      ...
      ['老板昵称',order.bossName],['老板编号',order.bossUid],
    ...
    body.innerHTML='<h2>订单详情</h2><p class="muted">'+esc(order.id)+' · '+esc(order.type)+' · '+esc(order.orderStatus)+'</p><div class="order-detail-tabs">'+tabs.map(function(tab){return '<span>'+esc(tab)+'</span>'}).join('')+'</div>'+timing+'<div class="detail-list">'+detail.map(function(item){return '<div><span>'+esc(item[0])+'</span><strong>'+esc(item[1])+'</strong></div>'}).join('')+'</div>'+adminDevInfoHtml([['Order UUID',order.rawId],['Boss Internal',orderValue(order,['boss_id','bossId'],'')],['Companion Internal',orderValue(order,['companion_id','companionId'],'')]])+'<div class="admin-sync-note">聊天记录、售后记录、支付流水和操作日志均应通过订单 ID 关联统一数据库；当前详情页不会生成模拟上下文。</div>';
    openAdminModal();
  }
  ...
  function renderPlayerManagement(){
    ...
    target.innerHTML='<div class="player-admin-page">'+
      '<div class="admin-section-head compact"><div><h3>陪玩列表</h3><p>仅显示审核通过的正式陪玩。申请草稿请到「申请草稿」；待审申请请到「陪玩申请审核」。</p></div><span class="admin-count-pill" data-player-count>0 条</span></div>'+
      '<div class="player-filter-panel">'+
        '<div class="player-filter-row player-filter-main">'+
          ...
          '<select data-player-filter="identity"><option value="">实名状态</option><option>已通过</option><option>待审核</option><option>已驳回</option><option>需要补资料</option><option>已认证</option><option>未认证</option></select>'+
          '<select data-player-filter="audit"><option value="">审核状态</option><option>未审核</option><option>待审核</option><option>审核中</option><option>已通过</option><option>已拒绝</option><option>已驳回</option><option>已停用</option></select>'+
          '<select data-player-filter="deposit"><option value="">押金状态</option><option>已缴纳</option><option>已到账</option><option>待审核</option><option>未缴纳</option><option>已驳回</option><option>已退回</option></select>'+
  ...
  function loadPlayerAdminRows(){
    adminFetch('/api/admin/players?scope=formal',{headers:{'x-mcj-admin-role':getRole(),Accept:'application/json'}}).then(function(res){
      var ct=res.headers.get('content-type')||'';
  ...
  function normalizePlayerAdmin(player){
    ...
    var internalId=player.id||player.playerId||player.player_id||'';
    var authUid=player.user_id||player.uid||player.auth_uid||'';
    var id=internalId||authUid||player.name||player.nickname;
    var companionCode=preferPublicCode([
      player.companion_code,player.companionCode,player.publicId,player.public_id
    ],'');
    var playerId=companionCode||'未分配';
    ...
    var audit=playerValue(player,['audit','auditStatus','audit_status','profileReviewStatusLabel','applicationStatus','reviewStatus'],'待审核');
    var identity=playerValue(player,['identityStatus','profileReviewStatusLabel','audit','auditStatus','applicationStatus','profile_review_status'],'待审核');
    if(/^(pending|approved|rejected|need_more|resubmit|verified)$/i.test(String(identity))){
      identity=({pending:'待审核',approved:'已通过',verified:'已通过',rejected:'已驳回',need_more:'需要补资料',resubmit:'需要补资料'})[String(identity).toLowerCase()]||identity;
    }
    var deposit=playerValue(player,['depositStatus','deposit_status','deposit'],'未缴纳');
    if(/^(pending|approved|rejected|unpaid|paid)$/i.test(String(deposit))){
      deposit=({pending:'待审核',approved:'已缴纳',paid:'已缴纳',rejected:'已驳回',unpaid:'未缴纳'})[String(deposit).toLowerCase()]||deposit;
    }
  ...
  function renderPlayerTableRows(){
    ...
    var headers=['头像','昵称','陪玩编号','登录邮箱','主接游戏','等级','实名','押金','抽成','状态','操作'];
    var body=pageRows.map(function(item){return '<tr class="player-list-row" data-player-open="'+esc(item.id)+'">'+
      ...
      '<td data-label="昵称" class="player-name-cell" title="'+esc(item.name)+'"><button class="player-name-link" type="button" data-player-action="view" data-player-id="'+esc(item.id)+'">'+esc(item.name)+'</button></td>'+
      playerTableCell('陪玩编号',esc(item.playerId))+
      playerTableCell('登录邮箱',esc(item.email))+
      playerTableCell('主接游戏',esc(item.mainGame))+
      playerTableCell('等级',esc(item.levelText))+
      '<td data-label="实名">'+statusChip(item.identity)+'</td>'+
      '<td data-label="押金">'+statusChip(item.deposit)+'</td>'+
      playerTableCell('抽成',esc(item.commission))+
      '<td data-label="状态">'+statusChip(item.account)+'</td>'+
      '<td data-label="操作" class="player-action-cell"><div class="player-ops"><button class="mini-btn" type="button" data-player-action="view" data-player-id="'+esc(item.id)+'">查看</button><button class="mini-btn primary-lite" type="button" data-player-action="edit" data-player-id="'+esc(item.id)+'">编辑</button><span class="player-more-wrap"><button class="mini-btn" type="button" data-player-more data-player-id="'+esc(item.id)+'">更多</button></span></div>'+adminDevInfoHtml([['Internal ID',item.internalId],['Auth UID',item.authUid],['开发邮箱',item.rawEmail&&isDevLogin(item.rawEmail)?item.rawEmail:'']])+'</td>'+
    '</tr>';}).join('');
  ...
  function exportPlayerRows(){
    var rows=visiblePlayerRows();
    var headers=['头像','陪玩名字','陪玩编号','登录邮箱','主接游戏','等级','资料审核状态','实名状态','押金状态','当前抽成','总收入','可提现余额','账号状态'];
    var lines=[headers].concat(rows.map(function(item){return [item.avatar,item.name,item.playerId,item.email,item.mainGame,item.levelText,item.audit,item.identity,item.deposit,item.commission,item.totalIncome,item.withdrawable,item.account]}));
    var csv=lines.map(function(line){return line.map(function(cell){return '"'+String(cell==null?'':cell).replace(/"/g,'""')+'"'}).join(',')}).join('\n');
  ...
  function playerDetailHtml(item,mode,focus){
    var raw=item.raw||{};
    var edit=mode==='edit';
    var basic=playerDetailRows([['头像',item.avatar],['昵称',item.name],['陪玩编号',item.playerId],['联系方式',item.phone],['登录邮箱',item.email],['注册时间',item.registered],['主接游戏',item.mainGame+' / '+item.gameId],['陪玩标签',item.tags.join('、')||'-']])+adminDevInfoHtml([['Internal ID',item.internalId],['Auth UID',item.authUid],['开发邮箱',item.rawEmail&&isDevLogin(item.rawEmail)?item.rawEmail:'']]);
  ...
  function openPlayerDetail(playerId,mode,focus){
    ...
    if(!useOverlay&&!drawer)return;
    var loadingHtml='<div class="player-drawer-head"><div><h2>陪玩详情</h2><p>正在读取真实资料…</p></div><button class="mini-btn" type="button" data-player-drawer-close>关闭</button></div><div class="admin-sync-note">资料加载中，请稍候。</div>';
    function mountHtml(html,title){
      if(useOverlay){
        if(!window.MCJAdminOverlay.isOpen())window.MCJAdminOverlay.open({title:title||'陪玩详情',html:html});
        else{window.MCJAdminOverlay.setBody(html);if(title)window.MCJAdminOverlay.setTitle(title);}
      }else{
        drawer.hidden=false;
        drawer.classList.add('open');
        drawer.innerHTML=html;
      }
      var root=useOverlay?window.MCJAdminOverlay.getBody():drawer;
      if(focus&&root){var section=root.querySelector('[data-player-detail-section="'+focus+'"]');if(section)section.scrollIntoView({block:'start'});}
    }
    mountHtml(loadingHtml,'陪玩详情');
    var Detail=window.MCJAdminPlayerDetail;
    var finish=function(detail,err){
      if(err){
        var safeMsg='资料暂时无法加载，请稍后再试。';
        var raw=String((err&&err.message)||err||'');
        if(/登录|未登录|unauthorized|401/i.test(raw))safeMsg='请先登录管理员账号。';
        mountHtml('<div class="player-drawer-head"><div><h2>陪玩详情</h2><p>资料加载失败</p></div><button class="mini-btn" type="button" data-player-drawer-close>关闭</button></div><div class="admin-sync-note">'+esc(safeMsg)+'</div><div class="player-drawer-actions"><button class="btn" type="button" data-player-action="view" data-player-id="'+esc(playerId)+'">重试</button><button class="btn" type="button" data-player-drawer-close>关闭</button></div>','陪玩详情');
        return;
      }
      var html,title='陪玩详情';
      if(Detail&&Detail.render){
        html=Detail.render(detail,mode||'view',focus||'');
        var item2=normalizePlayerAdmin(detail);
        title=(item2.name||'陪玩')+' · 详情';
      }else{
        var item=normalizePlayerAdmin(detail);
        title=(item.name||'陪玩')+' · 详情';
        html=playerDetailHtml(item,mode||'view',focus||'');
      }
      mountHtml(html,title);
    };
    if(Detail&&Detail.fetchDetail){
      Detail.fetchDetail(playerId).then(function(res){
        var detail=res.detail||res.player||res;
        // refresh list row cache from canonical detail fields
  ...
  function closePlayerDrawer(){
    closeAdminOverlay();
    var drawer=document.getElementById('playerDetailDrawer');
    if(drawer){drawer.hidden=true;drawer.classList.remove('open');drawer.innerHTML='';}
  }
  function collectPlayerEditForm(form){
    ...
    new FormData(form).forEach(function(value,key){
      if(key==='certTagIds'){
        if(!Array.isArray(data.certTagIds)) data.certTagIds=[];
        data.certTagIds.push(String(value));
        return;
      }
      data[key]=value;
    });
    // Explicit empty array when checkboxes exist but none checked
    if(form.querySelectorAll('input[name="certTagIds"]').length && !Array.isArray(data.certTagIds)){
      data.certTagIds=[];
    }
    return data;
  }
  ...
  function normalizeAdminConversation(item){
    item=item||{};
    var publicUid=preferPublicCode([
      item.bossUid,item.boss_uid,item.companionCode,item.companion_code,item.publicId,
      item.playerId,item.player_id,item.csCode,item.uid,item.userUid,item.user_uid,item.targetUid,item.target_uid
    ],'—');
    return {
      ...
      externalId:publicUid,
      internalId:item.targetId||item.target_id||item.user_id||item.userId||'',
  ...
  function renderAdminChatMain(active){
    ...
    return '<main class="admin-chat-main"><header class="admin-chat-header">'+(active?'<div class="admin-chat-title"><span class="admin-chat-avatar large">'+adminChatAvatar(active)+'<i class="'+(/在线|online/i.test(active.onlineStatus)?'online':'')+'"></i></span><div><strong>'+esc(active.name)+'</strong><small>编号 '+esc(active.uid)+' · '+esc(active.onlineStatus)+' · 当前负责客服：'+esc(active.assignedService)+'</small></div></div>':'<div class="admin-chat-title"><span class="admin-chat-avatar large"><span>喵</span></span><div><strong>请选择会话</strong><small>左侧选择真实聊天后查看记录</small></div></div>')+
  ...
  function renderAdminChatProfile(active){
    ...
    if(active&&/老板/.test(type)){
      base=base.concat([['老板编号',preferPublicCode([profile.bossId,profile.bossUid,active.externalId],'未设置')],['手机号',maskPhone(profile.phone||active.phone)],['VIP 等级',profile.vip||'-'],['余额',profile.balance||'RM0'],['最近订单',profile.recentOrder||'-'],['累计消费',profile.totalSpent||'RM0'],['累计充值',profile.totalRecharge||'RM0'],['邀请关系',profile.invite||'-'],['常玩游戏',profile.games||'-'],['游戏 ID',profile.gameId||'-'],['备注',profile.remark||'-']]);
    }else if(active&&/陪玩/.test(type)){
      base=base.concat([['陪玩编号',preferPublicCode([profile.companionCode,profile.publicId,active.externalId],'未分配')],['等级',profile.level||'-'],['当前单价',profile.price||'-'],['佣金',profile.commission||'-'],['返点',profile.rebate||'-'],['所属俱乐部',profile.club||'-'],['最近收入',profile.recentIncome||'RM0']]);
    }else if(active&&/客服/.test(type)){
      base=base.concat([['显示名称',profile.displayName||profile.name||active.name||'-'],['今日接单',profile.todayOrders||'0'],['工资',profile.salary||'RM0']]);
    }
  ...
  var platformContentModules={
    ...
    banners:{target:'crud-banners',title:'Banner 管理',type:'banners',desc:'已迁移至专用 Banner 管理模块（Supabase Storage）。',fields:['title','desktopImage','mobileImage','link','linkTarget','sort','startAt','endAt','autoPlay','intervalSeconds'],disabled:true},
    announcements:{target:'table-announcements',title:'公告管理',type:'announcements',desc:'对应首页 Banner 下方公告，启用后持续展示直至停用或删除。',fields:['category','audience','title','sort','scroll','content']},
    ...
    'team-lobby-links':{target:'teamLobbySettings',title:'组队大厅管理',type:'team_lobby_channels',desc:'管理组队大厅频道卡片（名称、Banner、简介、Discord/外部链接、排序、启用、首页显示）。前台 team-lobby 与首页入口按此读取，无硬编码链接。',fields:['name','image','description','discordUrl','sort','showOnHome']},
  ...
  function platformContentShell(cfg,items,meta){
    ...
    if(cfg.type==='team_lobby_channels'){
      var teamRows=items.map(function(item){
        var draft=contentDraft(item),pub=contentPublished(item);
        var data=Object.assign({},pub,draft);
        var image=data.image||data.cover||'';
        var showOnHome=data.showOnHome!==false&&data.showOnHome!=='false';
        var enabled=item.enabled!==false&&item.status!=='disabled'&&item.status!=='unpublished';
        return '<tr data-content-id="'+esc(item.id)+'"><td>'+esc(data.name||item.title||'-')+'</td><td>'+(image?'<img class="content-thumb" src="'+esc(image)+'" alt="">':'-')+'</td><td>'+esc(data.description||'-')+'</td><td>'+esc(data.discordUrl||data.discordLink||data.link||'-')+'</td><td>'+esc(item.sort!=null?item.sort:(data.sort||100))+'</td><td>'+statusChip(enabled?'启用':'停用')+'</td><td>'+statusChip(showOnHome?'显示':'隐藏')+'</td><td><div class="content-row-actions"><button class="mini-btn" data-content-action="edit" data-content-type="'+esc(cfg.type)+'" data-content-id="'+esc(item.id)+'" type="button">编辑</button><button class="mini-btn danger-btn" data-content-action="delete" data-content-type="'+esc(cfg.type)+'" data-content-id="'+esc(item.id)+'" type="button">删除</button></div></td></tr>';
      });
      return '<div class="platform-content-admin" data-platform-content="'+esc(cfg.type)+'">'+
        '<div class="content-admin-head"><div><h3>'+esc(cfg.title)+'</h3><p>'+esc(cfg.desc)+'</p></div><div class="content-version-meta"><span>最近保存：'+esc(meta&&meta.savedAt||'-')+'</span><span>前台同步：'+esc(meta&&meta.sync||'待发布')+'</span></div></div>'+
        '<div class="content-admin-toolbar"><input data-content-search="'+esc(cfg.type)+'" placeholder="搜索频道名称 / 链接"><button class="btn primary" data-content-action="new" data-content-type="'+esc(cfg.type)+'" type="button">新增</button><button class="btn" data-content-action="reload" data-content-type="'+esc(cfg.type)+'" type="button">刷新</button></div>'+
        '<div class="content-editor" data-content-editor="'+esc(cfg.type)+'" hidden></div>'+
        table(['频道名称','Banner','简介','Discord / 外部链接','排序','启用','首页显示','操作'],teamRows)+
        (!items.length?'<div class="content-empty-action"><strong>暂无组队频道</strong><span>新增并保存后，前台 team-lobby 立即读取同一份数据库内容（无硬编码默认频道）。</span><button class="btn primary" data-content-action="new" data-content-type="'+esc(cfg.type)+'" type="button">新增</button></div>':'')+
      '</div>';
    }
  ...
  function platformContentFieldLabel(cfg,field){
    if(cfg&&cfg.type==='team_lobby_channels'){
      if(field==='name')return '频道名称';
      if(field==='image')return 'Banner 图片';
      if(field==='description')return '简介';
      if(field==='discordUrl')return 'Discord 邀请链接（或其他外部链接）';
      if(field==='showOnHome')return '是否首页显示';
    }
    return contentFieldLabel(field);
  }
  function platformContentForm(cfg,item){
    item=item||{};
    var draft=Object.assign({},contentDraft(item));
    if(cfg.type==='team_lobby_channels'&&draft.showOnHome==null)draft.showOnHome=true;
    ...
    var inputs=fields.map(function(field){
      ...
      var fieldHtml=options?'<select name="'+esc(field)+'" data-admin-control="'+(isBoolContentField(field)?'switch':'select')+'">'+options.map(function(pair){return '<option value="'+esc(pair[0])+'" '+(String(value)===String(pair[0])?'selected':'')+'>'+esc(pair[1])+'</option>';}).join('')+'</select>':(isLong?'<textarea name="'+esc(field)+'">'+esc(value)+'</textarea>':'<input name="'+esc(field)+'" value="'+esc(value)+'"'+(field==='discordUrl'?' placeholder="https://discord.gg/... 或其它外部链接"':'')+'>');
      return '<label><span>'+esc(platformContentFieldLabel(cfg,field))+'</span>'+fieldHtml+(upload?'<input class="content-file" type="file" data-content-upload="'+esc(field)+'" accept="image/*,audio/*,application/pdf"><small>上传后会写入真实文件 URL</small>':'')+'</label>';
    }).join('');
    var directPublish=['banners','announcements','games','service_types','companion_tags','companion_levels','voice_types','availability_times','vip_levels','badges','featured_players','hot_games','player_rules','team_lobby_channels'].indexOf(cfg.type)>-1;
  ...
  function submitPlatformContent(action,type,id,payload){
    adminFetch('/api/admin/platform-content',{method:'POST',headers:{'Content-Type':'application/json','x-mcj-admin-role':getRole()},body:JSON.stringify({action:action,type:type,id:id||'',payload:payload||{}})}).then(function(res){return res.text().then(function(text){var body={};try{body=text?JSON.parse(text):{}}catch(parseErr){console.error('[Banner 管理] 保存接口非 JSON',{status:res.status,body:text,error:parseErr});throw new Error('平台内容接口返回非 JSON：HTTP '+res.status)}if(!res.ok||body.ok===false)throw new Error(body.message||('HTTP '+res.status));return body;})}).then(function(result){closeAdminOverlay();alert(type==='banners'?'Banner 已更新':(result.message||'已保存'));var cfg=platformContentConfig(type);if(cfg)loadPlatformContent(cfg);}).catch(function(err){console.error('[Banner 管理] 保存接口错误',{endpoint:'/api/admin/platform-content',action:action,type:type,id:id,error:err});if(isLocalPlatformContentType(type)){applyLocalPlatformContentAction(action,type,id||'',payload||{});closeAdminOverlay();alert(type==='banners'?'Banner 已保存到本地测试数据，首页刷新后同步显示。':'接口暂不可用，已使用当前后台保存方式保存内容。');var cfg=platformContentConfig(type);if(cfg)loadPlatformContent(cfg);return;}alert('操作失败：'+err.message+'。');});
  }
  function openPlatformContentEditor(type,id){
    var cfg=platformContentConfig(type);if(!cfg)return;
    function showEditor(item){
      var html=platformContentForm(cfg,item||null);
      var title=(item&&item.title)||extractModalTitle(html,cfg.title||'编辑内容');
      if(window.MCJAdminOverlay){window.MCJAdminOverlay.open({title:title,html:html});return;}
      var target=document.querySelector('[data-content-editor="'+type+'"]');if(!target)return;
      target.hidden=false;target.innerHTML=html;target.scrollIntoView({behavior:'smooth',block:'nearest'});
    }
    if(!id){showEditor(null);return;}
    function openItem(item){showEditor(item||{});}
  ...
  function openHomeEntryEditor(slug){
    apiFetchPlatformContent('homepage_entries').then(function(result){
      ...
      var html=homeEntryEditor(entry);
      var title='编辑：'+(entry.name||'入口');
  ...
  function homeEntryEditor(entry){
    var special='';
    if(entry.slug==='team-lobby')special='<section class="home-entry-special"><h3>组队频道</h3><p>频道名称、Banner、简介、Discord/外部链接、排序、启用与首页显示，请到左侧导航「组队大厅管理」维护。此处只配置首页入口跳转 team-lobby.html。</p><button class="btn primary" type="button" data-team-lobby-admin-jump>打开组队大厅管理</button></section>';
  ...
  function submitHomeEntryForm(form){
    ...
    savePlatformContentStrict(form.dataset.homeEntryId?'save':'create','homepage_entries',form.dataset.homeEntryId,payload).then(function(){closeAdminOverlay();alert('已保存并应用');renderHomeEntryManager();}).catch(function(err){console.error('[首页入口管理] 保存失败',{type:'homepage_entries',error:err});alert('保存失败：'+err.message);});
  }
  function teamChannelDefaults(){return [];}
  function normalizeTeamChannel(row){
    var draft=row&&row.draft?row.draft:{};
    var published=row&&row.published?row.published:{};
    var data=Object.assign({},published,draft);
    var slug=String(data.slug||row.slug||'').trim();
    var showOnHome=data.showOnHome;
    if(showOnHome==null)showOnHome=true;
    return {id:row&&row.id||'',slug:slug,name:data.name||row.title||'未命名频道',description:data.description||'',discordUrl:data.discordUrl||data.discordLink||data.link||'',image:data.image||'',sort:Number(row&&row.sort!=null?row.sort:(data.sort!=null?data.sort:100)),enabled:row?row.enabled!==false&&row.status!=='disabled'&&row.status!=='unpublished':true,showOnHome:showOnHome===true||showOnHome==='true'};
  }
  function loadHomeTeamChannels(){
    ...
    apiFetchPlatformContent('team_lobby_channels').then(function(result){
      var channels=(Array.isArray(result.items)?result.items:[]).map(normalizeTeamChannel).sort(function(a,b){return Number(a.sort||100)-Number(b.sort||100)});
      target.innerHTML=(channels.length?channels.map(teamChannelCard).join(''):'<div class="empty">暂无组队频道，请到「组队大厅管理」新增。</div>')+'<div class="home-entry-special" style="grid-column:1/-1"><button class="btn" type="button" data-team-lobby-admin-jump>打开完整组队大厅管理（新增/删除）</button></div>';
  ...
  function teamChannelCard(item){
    return '<form class="home-channel-card" data-home-channel-form="'+esc(item.slug||item.id||'')+'" data-home-channel-id="'+esc(item.id||'')+'"><div class="home-channel-preview">'+(item.image?'<img src="'+esc(item.image)+'" alt="">':'<span>频道</span>')+'</div><label><span>频道名称</span><input name="name" value="'+esc(item.name)+'"></label><label><span>简介</span><input name="description" value="'+esc(item.description)+'"></label><label><span>Banner 图片</span><input name="image" value="'+esc(item.image||'')+'"></label><label><span>Discord / 外部链接</span><input name="discordUrl" value="'+esc(item.discordUrl||'')+'" placeholder="留空时前台显示暂未开放"></label><label><span>排序</span><input name="sort" type="number" value="'+esc(item.sort||100)+'"></label><label><span>是否启用</span><select name="enabled"><option value="true" '+(item.enabled?'selected':'')+'>启用</option><option value="false" '+(!item.enabled?'selected':'')+'>停用</option></select></label><label><span>是否首页显示</span><select name="showOnHome"><option value="true" '+(item.showOnHome?'selected':'')+'>显示</option><option value="false" '+(!item.showOnHome?'selected':'')+'>隐藏</option></select></label><button class="btn primary" type="submit">保存频道</button></form>';
  }
  function submitHomeChannelForm(form){
    var data={};new FormData(form).forEach(function(value,key){data[key]=value;});data.slug=form.dataset.homeChannelForm||('channel-'+Date.now());data.sort=Number(data.sort||100);data.enabled=data.enabled!=='false';data.showOnHome=data.showOnHome!=='false';
    var payload={slug:data.slug,title:data.name,status:'published',enabled:data.enabled,sort:data.sort,draft:data};
    savePlatformContentStrict(form.dataset.homeChannelId?'save':'create','team_lobby_channels',form.dataset.homeChannelId,payload).then(function(){alert('频道已保存');loadHomeTeamChannels();var cfg=platformContentConfig('team_lobby_channels');if(cfg)loadPlatformContent(cfg);}).catch(function(err){console.error('[首页入口管理] 保存组队频道失败',{type:'team_lobby_channels',error:err});alert('保存失败：'+err.message);});
  }
  ...
  var sectionTitles={
    ...
    'companion-tags':['陪玩标签管理','维护随和、技术流等普通标签；声线请用声线管理'],
    ...
    'team-lobby-links':['组队大厅管理','频道名称、Banner、简介、Discord/外部链接、排序、启用与首页显示'],
    ...
    'companion-drafts':['申请草稿','未提交审核的陪玩申请草稿（不进入正式列表）'],
    'service-accounts':['客服管理','创建、启用和停用客服登录账号'],
    'service-stats':['客服工资中心','接待、订单与工资估算'],
    'service-commission':['客服佣金设置','全局 RM 佣金、底薪与结算规则'],
    'service-attendance':['客服打卡查询','永久打卡历史与工时统计'],
  ...
  function initTabs(){
    ...
    var orderSectionAliases={'order-waiting-accept':'orders','order-waiting-confirm':'orders','order-running':'orders','order-completed':'orders','refund-orders':'orders','pending-orders':'orders','after-sale-orders':'orders','service-stats':'service-accounts','service-commission':'service-accounts','service-attendance':'service-accounts'};
  ...
  function bindGlobal(){document.addEventListener('click',function(e){var role=e.target.closest('[data-role-login]');if(role){/* P0: never fake-switch roles without real login */location.replace('/admin/login/');return;}var logout=e.target.closest('[data-admin-logout]');if(logout){['mcjRole','adminAuthToken','adminUser','mcjAuthAccessToken','mcjAuthRefreshToken','mcjAuthExpiresAt','mcjAfterLoginRedirect'].forEach(function(k){try{localStorage.removeItem(k);sessionStorage.removeItem(k);}catch(err){}});if(window.MCJRoleGate&&window.MCJRoleGate.logout)window.MCJRoleGate.logout('admin');location.replace('/admin/login/');return;}var preview=e.target.closest('[data-preview-home]');if(preview){location.href='index.html';return;}var saveLevels=e.target.closest('[data-save-companion-levels]');if(saveLevels&&levelApi()){levelApi().save(collectCompanionLevels());log('保存陪玩等级与价格设置');alert('已保存陪玩等级与价格设置');renderCompanionLevels();return;}var deleteLevel=e.target.closest('[data-delete-companion-level]');if(deleteLevel&&levelApi()){var levels=getLevels();var level=levelApi().find(deleteLevel.dataset.deleteCompanionLevel);if(playerLevelCount(level)>0){alert('该等级已有陪玩，不能直接删除。请先停用该等级或迁移陪玩等级。');return;}if(confirm('确认删除 '+levelLabel(level.id)+'？')){levelApi().save(levels.filter(function(item){return item.id!==level.id}));log('删除陪玩等级 '+levelLabel(level.id));renderCompanionLevels();}return;}var action=e.target.closest('[data-action]');if(action){alert('已执行：'+action.dataset.action+' / '+(action.dataset.id||''));log('执行 '+action.dataset.action);return;}var del=e.target.closest('[data-delete]');if(del){var arr=read(del.dataset.delete);arr.splice(Number(del.dataset.index),1);write(del.dataset.delete,arr);location.reload();return;}})}
  ...
  function bindPaymentAdmin(){
    ...
    document.addEventListener('click',function(e){
      ...
      var jump=e.target.closest('.todo-item[data-section],.activity-item[data-section]');if(jump){if(window.MCJAdminOverlay&&window.MCJAdminOverlay.isOpen())closeAdminOverlay();var nav=document.querySelector('.side-nav [data-section="'+jump.dataset.section+'"]');if(nav)nav.click();return;}
      var dashPendingOpen=e.target.closest('[data-dashboard-pending-open]');if(dashPendingOpen){renderDashboardPendingOverlay(1);return;}
      var dashPendingPage=e.target.closest('[data-dashboard-pending-page]');if(dashPendingPage){var dir=dashPendingPage.dataset.dashboardPendingPage;var next=dashboardState.pendingPage+(dir==='next'?1:-1);renderDashboardPendingOverlay(next);return;}
      var dashTrends=e.target.closest('[data-dashboard-trends]');if(dashTrends){openDashboardTrendsOverlay();return;}
      ...
      var orderAction=e.target.closest('[data-order-action]');if(orderAction){var orderAct=orderAction.dataset.orderAction,orderId=orderAction.dataset.orderId;if(orderAct==='view'){openOrderDetail(orderId);return;}var payload={};if(orderAct==='assign-player'||orderAct==='change-player'){var companionId=prompt('请输入陪玩编号（如 PW00001）或昵称：')||'';if(!String(companionId).trim())return;payload.companion_id=String(companionId).trim();payload.companion_code=String(companionId).trim();}var risky=/cancel|refund|early-end|confirm-complete|return-service|blacklist|compensate|reject|approve|partial/.test(orderAct);var reason='';if(risky){reason=prompt('该订单操作需要记录原因，请填写原因：')||'';if(!reason.trim())return;payload.reason=reason;}submitOrderAction(orderAct,orderId,payload);return;}
      ...
      if(e.target.closest('[data-order-dev-test]')){return;}
      ...
      if(playerMorePopover.el&&!e.target.closest('[data-player-more-popover]')&&!e.target.closest('[data-player-more]')){
        closePlayerMoreMenu();
      }
      ...
      var playerPage=e.target.closest('[data-player-page]');if(playerPage){closePlayerMoreMenu();playerAdminState.page+=playerPage.dataset.playerPage==='next'?1:-1;renderPlayerTableRows();return;}
      var playerPageGo=e.target.closest('[data-player-page-go]');if(playerPageGo){closePlayerMoreMenu();var jump=document.querySelector('[data-player-page-jump]');var total=visiblePlayerRows().length;var pages=Math.max(1,Math.ceil(total/playerAdminState.pageSize));playerAdminState.page=Math.min(Math.max(1,Number(jump&&jump.value)||1),pages);renderPlayerTableRows();return;}
      var playerClose=e.target.closest('[data-player-drawer-close]');if(playerClose){closePlayerDrawer();return;}
      var playerAction=e.target.closest('[data-player-action]');if(playerAction){var action=playerAction.dataset.playerAction,id=playerAction.dataset.playerId,focus=playerAction.dataset.playerSection||'';closePlayerMoreMenu();if(action==='view'||action==='edit'){openPlayerDetail(id,action,focus);return;}if(action==='save-detail'){var detailForm=document.querySelector('[data-player-detail-form]');var payload=collectPlayerEditForm(detailForm);if(window.MCJAdminPlayerDetail&&window.MCJAdminPlayerDetail.isSaving&&window.MCJAdminPlayerDetail.isSaving())return;savePlayerAdminChanges('edit',id,payload,function(result){alert((result&&result.message)||'修改已保存');openPlayerDetail(id,'view',focus);});return;}if(action==='save-edit'){submitPlayerSecure('edit',id,collectPlayerEditForm(document.querySelector('[data-player-edit-form]')));return;}if(/freeze|ban-order|disable/.test(action)){if(!confirm(action==='disable'?'确认删除/停用该陪玩账号？此操作会停用账号并禁止接单。':'确认封禁该陪玩账号？'))return;submitPlayerSecure(action,id,{});return;}submitPlayerSecure(action,id,{});return;}var playerOpen=e.target.closest('[data-player-open]');if(playerOpen&&!e.target.closest('button,a,input,select')){openPlayerDetail(playerOpen.dataset.playerOpen,'view');return;}
    ...
    document.addEventListener('keydown',function(e){
      if(e.key==='Escape'){
        if(window.MCJAdminOverlay&&window.MCJAdminOverlay.isOpen()){if(adminModalOverlayBridge)closeAdminModal();else closeAdminOverlay();return;}
        var adminModal=document.getElementById('adminModal');
  ...
  function renderDashboardPendingOverlay(page){
    if(!window.MCJAdminOverlay)return;
    var items=dashboardState.pendingItems;
    var pageSize=dashboardState.pendingPageSize;
    var pages=Math.max(1,Math.ceil(items.length/pageSize));
    var p=Math.min(Math.max(1,page||dashboardState.pendingPage||1),pages);
    dashboardState.pendingPage=p;
    var slice=items.slice((p-1)*pageSize,p*pageSize);
    var listHtml='<div class="todo-list">'+slice.map(function(item){return '<button class="todo-item" type="button" data-section="'+item[2]+'"><span class="todo-icon">'+item[1]+'</span><span><strong>'+esc(item[0])+'</strong><span>点击进入对应模块处理</span></span><span class="status '+(item[1]?'wait':'ok')+'">'+(item[1]?'待处理':'已清空')+'</span></button>';}).join('')+'</div>';
    var pager=pages>1?'<div class="table-footer dashboard-pending-pager"><button class="mini-btn" type="button" data-dashboard-pending-page="prev" '+(p<=1?'disabled':'')+'>上一页</button><span>第 '+p+' / '+pages+' 页</span><button class="mini-btn" type="button" data-dashboard-pending-page="next" '+(p>=pages?'disabled':'')+'>下一页</button></div>':'';
    var html=listHtml+pager;
    if(window.MCJAdminOverlay.isOpen())window.MCJAdminOverlay.setBody(html);
    else window.MCJAdminOverlay.open({title:'待处理事项',html:html});
  }
  function openDashboardTrendsOverlay(){
    if(!window.MCJAdminOverlay)return;
    var html='<div class="admin-chart-grid dashboard-trends">'+
      dashboardTrendCard('今日订单趋势',dashboardState.orderSeries,'待系统产生真实订单后自动生成趋势图。')+
      dashboardTrendCard('营业额趋势',dashboardState.revenueSeries,'待系统产生真实订单后自动生成趋势图。')+
      dashboardTrendCard('平台利润趋势',dashboardState.profitSeries,'待系统产生真实利润记录后自动生成趋势图。')+
    '</div>';
    window.MCJAdminOverlay.open({title:'数据趋势',html:html});
  }
  function renderDashboardExperience(){
    ...
    if(pending){
      ...
      pending.innerHTML='<div class="dashboard-pending-summary"><span>待处理 <strong data-dash-pending-count>…</strong> 项</span><div class="admin-dash-actions"><button class="mini-btn primary-lite" type="button" data-dashboard-pending-open>打开待处理</button><button class="mini-btn" type="button" data-dashboard-trends>查看趋势</button></div></div>';
      function fillPending(items){
        dashboardState.pendingItems=items;
        dashboardState.totalPending=items.reduce(function(sum,item){return sum+Number(item[1]||0);},0);
        var countEl=pending.querySelector('[data-dash-pending-count]');
        if(countEl)countEl.textContent=String(dashboardState.totalPending);
      }
      // Prefer real API counts; fall back to local mirrors.
      Promise.resolve()
        .then(function(){
          if(!Auth||!Auth.get)throw new Error('no auth');
          return Promise.all([
            Auth.get('/api/admin/players?scope=applications&limit=200').catch(function(){return {}}),
            Auth.get('/api/admin/finance?action=bootstrap').catch(function(){return {}}),
            Auth.get('/api/admin/orders?limit=200').catch(function(){return {}}),
            Auth.get('/api/admin/dashboard').catch(function(){return {}})
          ]);
        })
        .then(function(resList){
          var players=resList[0].players||resList[0].rows||[];
          var finance=resList[1]||{};
          var orders=resList[2].orders||[];
          var dash=resList[3].stats||{};
          var pendingPlayers=players.filter(function(x){
            var st=String(x.application_status||x.applicationStatus||x.audit||x.status||'');
            return /pending|待|审核|submitted|review|resubmit|need_more/i.test(st) && !/draft|archived|已通过|approved/i.test(st);
          }).length;
          var pendingWithdraw=(finance.withdrawals||finance.withdraw_requests||[]).filter(function(x){
            return /pending|待|审核/i.test(String(x.status||''));
          }).length;
          var pendingRefund=orders.filter(function(o){return o.status==='refund_requested'||o.status==='after_sale'}).length;
          var afterSale=orders.filter(function(o){return o.status==='refund_requested'||o.status==='after_sale'}).length;
          var awaitingCs=Number(dash.awaitingPayment||0)+Number(dash.pendingOrders||0);
          fillPending([
            ['待审核陪玩',pendingPlayers,'companion-applications'],
            ['待审核提现',pendingWithdraw,'service-reports'],
            ['待退款',pendingRefund,'orders'],
            ['待处理售后',afterSale,'orders'],
            ['待客服处理',awaitingCs,'service-accounts']
          ]);
        })
        .catch(function(){
          var players=read('players'), withdraws=read('withdraw_requests'), refunds=read('refunds'), tickets=read('customer_tickets');
          fillPending([
            ...
            ['待退款',refunds.filter(function(x){return /待|退款中|处理中/.test(String(x.status||''))}).length,'orders'],
            ['待处理售后',refunds.filter(function(x){return /售后|处理中/.test(String(x.status||''))}).length,'orders'],
            ['待客服处理',tickets.filter(function(x){return /待|处理中/.test(String(x.status||''))}).length,'service-accounts']
          ]);
        });
    }
    var logsTarget=document.getElementById('table-admin_logs');
    if(logsTarget){
      var logs=read('admin_logs').slice(0,5);
    ...
    dashboardState.orderSeries=dashboardSeries(orders,function(){return 1});
    dashboardState.revenueSeries=dashboardSeries(orders,function(row){return moneyNumber(row.actual_paid||row.actualPaid||row.paid_amount||row.amount||row.total)});
    dashboardState.profitSeries=dashboardSeries(orders.concat(walletRows),function(row){return moneyNumber(row.platform_profit||row.platformProfit||row.profit||0)});
    // trends button is inside pending summary; no extra long page block
  }
  ...
  function v1AccountFormHtml(item){
    item=item||{};
    var role=item.role||'service';
    var status=item.status||'ENABLED';
    return '<form class="admin-account-form" data-v1-account-form><input type="hidden" name="id" value="'+esc(item.id||'')+'"><div class="form-grid"><label>&#22995;&#21517;<input name="name" required value="'+esc(item.name||'')+'"></label><label>&#36134;&#21495;<input name="account" required value="'+esc(item.account||'')+'"></label><label>&#23494;&#30721;<input name="password" type="password" placeholder="&#26032;&#24314;&#24517;&#22635;&#65292;&#32534;&#36753;&#21487;&#30041;&#31354;"></label><label>&#36523;&#20221;<select name="role" required><option value="boss"'+(role==='boss'?' selected':'')+'>&#32769;&#26495;</option><option value="service"'+(role==='service'?' selected':'')+'>&#23458;&#26381;</option><option value="companion"'+(role==='companion'?' selected':'')+'>&#38506;&#29609;</option></select></label><label>&#29366;&#24577;<select name="status"><option value="ENABLED"'+(status==='ENABLED'?' selected':'')+'>&#21551;&#29992;</option><option value="DISABLED"'+(status==='DISABLED'?' selected':'')+'>&#20572;&#29992;</option></select></label></div><div class="row"><button class="primary-btn" type="submit">&#20445;&#23384;&#36134;&#21495;</button><button class="ghost-btn" type="button" data-v1-cancel-account>&#21462;&#28040;</button></div></form>';
  }
  function openV1AccountEditor(item){
    var html=v1AccountFormHtml(item||{});
    var title=item&&item.id?'\u7f16\u8f91\u8d26\u53f7':'\u65b0\u589e\u8d26\u53f7';
    if(window.MCJAdminOverlay){window.MCJAdminOverlay.open({title:title,html:html});return;}
    var target=document.getElementById('table-admin_accounts');
    if(!target)return;
    var existing=target.querySelector('[data-v1-account-form]');
    if(existing&&existing.parentNode)existing.parentNode.removeChild(existing);
    var toolbar=target.querySelector('.table-tools');
    if(toolbar)toolbar.insertAdjacentHTML('afterend',html);
    var form=target.querySelector('[data-v1-account-form]');
    if(form)form.scrollIntoView({block:'nearest'});
  }
  function resetV1AccountForm(){
    if(window.MCJAdminOverlay&&window.MCJAdminOverlay.isOpen&&window.MCJAdminOverlay.isOpen()){window.MCJAdminOverlay.close();return;}
    var form=document.querySelector('[data-v1-account-form]');
    if(form&&form.parentNode)form.parentNode.removeChild(form);
  }
  function bindV1AccountManagement(){
    document.addEventListener('click',function(e){
      if(e.target.closest('[data-v1-new-account]')){openV1AccountEditor({role:'service',status:'ENABLED'});return;}
      if(e.target.closest('[data-v1-cancel-account]')){resetV1AccountForm();return;}
      var edit=e.target.closest('[data-v1-edit-account]');
      if(edit){var list=v1Read('mcj_v1_accounts',[]),item=list.find(function(x){return x.id===edit.dataset.v1EditAccount});if(item)openV1AccountEditor(item);return;}
  ...
  function initSuperAdmin(){
    // #superStats and #dashboardPending are owned by admin-final-v1.js renderDashboard()
    // and renderDashboardExperience() (real API data). Do not write stale localStorage
    // snapshots here — that used to overwrite the real numbers after they loaded.
    var tables={
    ...
    var rechargeAlt=document.getElementById('table-recharge_requests_alt');
    // Real payment_orders queue is owned by admin-recharge-review.js — do not paint mock rows over it.
    if(rechargeAlt&&!document.getElementById('rechargeReviewMount'))renderGenericTable('recharge_requests',rechargeAlt,tables.recharge_requests);
