(function () {
  ...
  function hasAuthSession() {
    // Boss identity only — never treat CS/companion/admin shared JWT as boss login.
    ...
    var role = String(localStorage.getItem("mcjRole") || sessionStorage.getItem("mcjRole") || "").toLowerCase();
    if (role && role !== "boss" && role !== "customer") return false;
    return !!(
      (accessToken() && (role === "boss" || role === "customer")) ||
  ...
  function scheduleAuthVisibility() {
    applyAuthVisibility();
    // Auth rebuild can orphan an open mobile-nav sheet (backdrop stays, scroll locked).
    closeNavMoreMenu();
  ...
  function mount() {
    ...
    ensureCss("/src/boss-header.css?v=20260803overlayP0", "data-mcj-boss-header-css");
  ...
  function openLogin() {
    if (window.MCJAuthContinue && typeof window.MCJAuthContinue.openLogin === "function") {
      window.MCJAuthContinue.openLogin("login");
      return;
    }
