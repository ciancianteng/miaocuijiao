(function () {
  ...
  /** In-memory catalog — API hydrate wins; localStorage is offline fallback only. */
  var memoryLevels = null;
  var memoryPublishedAt = "";
  ...
  function read() {
    if (memoryLevels && memoryLevels.length) {
      applyTheme(memoryLevels);
      return memoryLevels.slice();
    }
  ...
  function themeCss(levels) {
    return (levels || []).map(function (level) {
      var item = normalizeLevelRecord(level);
      var id = item.id;
      var photoBorder = item.badgeBorder || item.color;
      return [
        '[data-companion-level="' + id + '"],.player-card[data-level-id="' + id + '"],.companion-level-pill[data-level-id="' + id + '"],.mcj-level-tag[data-level-id="' + id + '"]{--mcj-level-color:' + item.color + ';--mcj-level-display:' + item.displayColor + ';--mcj-level-badge-border:' + item.badgeBorder + ';--mcj-level-badge-text:' + item.badgeText + ';--mcj-level-badge-icon:' + item.badgeIcon + ';}',
        '.player-card[data-level-id="' + id + '"]{border-color:' + item.color + '55!important;background:' + cardBackgroundCss(item) + '!important;}',
        '.player-card[data-level-id="' + id + '"] .companion-level-pill,.companion-level-pill[data-level-id="' + id + '"],[data-companion-level="' + id + '"] .companion-level-pill,.mcj-level-tag[data-level-id="' + id + '"]{border:1.5px solid ' + item.badgeBorder + '!important;color:' + item.badgeText + '!important;background:' + item.color + '22!important;}',
        '[data-companion-level="' + id + '"] .profile-avatar-wrap,.player-card[data-level-id="' + id + '"] .companion-avatar,.player-card[data-level-id="' + id + '"] .avatar{border-color:' + photoBorder + 'aa!important;box-shadow:0 0 18px ' + photoBorder + '44!important;}',
        '.player-card[data-level-id="' + id + '"] .companion-card-media{box-shadow:inset 0 0 0 1.5px ' + photoBorder + '88;}',
  ...
  function hydrateFromList(list, meta) {
    if (!Array.isArray(list) || !list.length) return read();
    if (meta && meta.publishedAt) memoryPublishedAt = String(meta.publishedAt);
    return save(list.map(normalizeLevelRecord));
  }

  function hydrateFromApi() {
    return fetch("/api/platform/companion-levels?all=1&_=" + Date.now(), {
      ...
      .then(function (res) {
        return res.json().catch(function () { return {}; }).then(function (body) {
          ...
          return hydrateFromList(body.levels || [], { publishedAt: body.publishedAt || body.updated_at || "" });
        });
      })
      .catch(function () {
        return read();
  ...
  // Defaults for first paint; always refresh from platform API (DB source of truth).
  read();
  if (typeof window !== "undefined") {
    hydrateFromApi();
    document.addEventListener("visibilitychange", function () {
      if (document.visibilityState === "visible") hydrateFromApi();
    });
  }
  ...
  window.MCJCompanionLevels = {
    ...
    publishedAt: function () { return memoryPublishedAt; }
  };
})();
