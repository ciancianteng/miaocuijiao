...
(function () {
  ...
  function syncEditorOverlays() {
    ...
    if (state.tab === "channels" && state.editId) {
      var channel = channelById(state.editId);
      if (channel) {
        var channelHtml = renderEditor(channel);
        var channelTitle = "编辑 · " + (channel.name || "");
        if (window.MCJAdminOverlay.isOpen && window.MCJAdminOverlay.isOpen()) {
          window.MCJAdminOverlay.setTitle(channelTitle);
          window.MCJAdminOverlay.setBody(channelHtml);
        } else {
    ...
    if (state.tab === "banks" && state.bankFormOpen) {
      var bankHtml = renderBankEditor(bankById(state.bankEditId));
      var bankTitle = state.bankEditId ? "编辑收款渠道" : "新增收款渠道";
      if (window.MCJAdminOverlay.isOpen && window.MCJAdminOverlay.isOpen()) {
        window.MCJAdminOverlay.setTitle(bankTitle);
        window.MCJAdminOverlay.setBody(bankHtml);
      } else {
        window.MCJAdminOverlay.open({
          title: bankTitle,
          html: bankHtml,
  ...
  function closeEditorOverlay(clearFn) {
  ...
  function renderChannels() {
    ...
    var editor = !window.MCJAdminOverlay && state.editId ? renderEditor(channelById(state.editId)) : "";
    return (
  ...
  function renderBanks() {
    ...
    var editor = !window.MCJAdminOverlay && state.bankFormOpen ? renderBankEditor(bankById(state.bankEditId)) : "";
    return (
