...
import { formatBossCode, parseBossCodeNumber } from "./_account-codes.js";
import {
...
async function getOrCreateConversation(profile, orderId = "", meta = {}) {
  ...
  const consultType = normalizeBossConsultType(meta?.consultType || meta?.consult_type, { orderId });
  const consultLabel = consultTypeLabel("boss", consultType);
  if (orderId) {
    const owned = await supabaseJson(
  ...
  // Reuse ONLY same boss + order_id + consult_type. Different order/consult → new conversation_id.
  if (!forceNew) {
    const base = orderId
      ? `?boss_id=eq.${encodeURIComponent(profile.id)}&order_id=eq.${encodeURIComponent(orderId)}&status=not.in.(closed,ended)`
      : `?boss_id=eq.${encodeURIComponent(profile.id)}&order_id=is.null&status=not.in.(closed,ended)`;
    const withConsult = `${base}&consult_type=eq.${encodeURIComponent(consultType)}&order=updated_at.desc&limit=1`;
    let active = await supabaseJson(restUrl("conversations", withConsult), { headers: serviceHeaders() }).catch((err) => {
      if (/consult_type|column|schema/i.test(String(err?.message || ""))) return null;
      return [];
    });
    if (active === null) {
      // Column missing: fall back to order-scoped reuse only (still not account-wide without order).
      active = await supabaseJson(restUrl("conversations", `${base}&order=updated_at.desc&limit=1`), {
        headers: serviceHeaders(),
      }).catch(() => []);
    }
  ...
  // conversations schema: boss_id / companion_id / customer_service_id / order_id / status / conversation_type / consult_type
  ...
  try {
    ...
    // Fallback if consult_type / conversation_type column missing in an older DB.
    if (/consult_type|conversation_type/i.test(msg)) {
      const legacy = { ...payload };
      if (/consult_type/i.test(msg)) delete legacy.consult_type;
      if (/conversation_type/i.test(msg)) delete legacy.conversation_type;
      rows = await supabaseJson(restUrl("conversations"), {
        method: "POST",
        headers: serviceHeaders(),
  ...
  if (conversation?.id) {
    const tip = order
      ? forceNew
        ? `老板重新发起订单咨询（${consultLabel}），已关联订单 ${order.order_no || order.id}。`
        : `老板发起客服咨询（${consultLabel}），已自动关联订单 ${order.order_no || order.id}。`
      : forceNew
        ? `老板重新发起人工客服咨询（${consultLabel}）。`
        : `老板发起客服咨询（${consultLabel}）。`;
...
function serviceStatusText(conversation, presence, serviceName) {
  if (isPendingTransferStatus(conversation?.status) || (!conversation?.customer_service_id && conversation?.status === "pending_transfer")) {
    return TRANSFER_USER_TIP;
  }
  ...
  return {
    conversation: conversation
      ? Object.assign({}, conversation, {
          ...
          consultType: conversation.consult_type || (conversation.order_id ? "current_order" : "other"),
          consultTypeLabel: consultTypeLabel(
            "boss",
            conversation.consult_type || (conversation.order_id ? "current_order" : "other")
          ),
...
async function listConversations(profile) {
  ...
  return conversations.map((row) => {
    ...
    return {
      ...
      consultType: row.consult_type || (row.order_id ? "current_order" : "other"),
      consultTypeLabel: consultTypeLabel("boss", row.consult_type || (row.order_id ? "current_order" : "other")),
      status: row.status || "",
...
async function resolveConversation(profile, { conversationId = "", orderId = "", create = true, forceNew = false, consultType = "" } = {}) {
  ...
  return getOrCreateConversation(profile, orderId || "", { forceNew: !!forceNew, consultType });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const resolved = await resolveConversation(profile, {
      ...
      orderId,
      create: shouldCreate,
      consultType: String(body.consult_type || body.consultType || req.query?.consult_type || "").trim(),
    });
