...
async function loadCompanionServices(companionUserId, companionRow) {
  ...
  if (rows?.length) {
    return rows
      ...
      .filter((s) => s.name && money(s.price) > 0);
  }
  // Fallback: one service per enabled game with its own price (boss picks game → auto price)
  let catalog = [];
  try {
    const bundle = await loadPublicServices();
    catalog = Array.isArray(bundle?.services) ? bundle.services : [];
  } catch {
    catalog = [];
  }
  const fromGames = servicesFromGamePrices(companionRow || {}, catalog);
  ...
  return fromGames
    ...
    .filter((s) => s.name && money(s.price) > 0 && !/^[0-9a-f-]{36}$/i.test(String(s.name)));
}
