...
async function syncBannerToPublicTable(item) {
  ...
  const image =
    String(src.desktopImage || src.image || src.image_url || src.imageUrl || "").trim() ||
    String(src.mobileImage || "").trim();
  const mobileImage = String(src.mobileImage || src.mobile_image_url || src.mobileImageUrl || "").trim();
...
function shouldAutoPublish(type) {
  return type === "banners" || type === "player_rules" || type === "team_lobby_channels";
}
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (action === "create") {
        ...
        if (payload.id) item.id = String(payload.id);
        if (!item.id) item.id = `pc-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        if (shouldAutoPublish(item.type) && item.enabled !== false) {
        ...
        try {
          ...
          await writeLog(req, shouldAutoPublish(item.type) ? "create_publish" : "create", item.type, saved?.id, null, saved);
          return json(res, 200, { ok: true, message: shouldAutoPublish(item.type) ? "已保存并同步到前台" : "已保存", item: saved });
        } catch (error) {
      ...
      if (action === "save") {
        ...
        if (shouldAutoPublish(before.type) && item.enabled !== false) {
          item.status = "published";
          item.published = item.draft || {};
        ...
        try {
          ...
          const saved = rows?.[0] || item;
          await syncPublicFrontTables(saved);
          await writeLog(req, shouldAutoPublish(before.type) ? "save_publish" : "save_draft", before.type, id, before, saved);
          return json(res, 200, { ok: true, message: shouldAutoPublish(before.type) ? "已保存并同步到前台" : "已保存", item: saved });
        } catch (error) {
          if (!isMissingTableError(error)) throw error;
          const saved = await upsertAnnouncementItem(item);
          await syncPublicFrontTables(saved);
