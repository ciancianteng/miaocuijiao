...
import { requireAdmin as requireAdminJwt, ADMIN_ROLES } from "../_admin-auth.js";
import {
  CATEGORIES,
  PRICING_UNITS,
  normalizeProductRow,
...
const GAMEPLAY_ADMIN_ROLES = new Set(["admin", "super_admin", "content_admin"]);
...
function tableMissingError() {
  return Object.assign(
    new Error("gameplay_products 表未初始化，无法写入。请执行 supabase/gameplay-products.sql 后重试。"),
    { status: 503 }
  );
}
...
async function requireAdmin(req) {
  try {
    return await requireAdminJwt(req, { allowRoles: GAMEPLAY_ADMIN_ROLES.size ? GAMEPLAY_ADMIN_ROLES : ADMIN_ROLES });
  } catch (error) {
    const status = Number(error?.status) || 403;
    if (status === 401) throw Object.assign(new Error(error.message || "请先登录管理员账号。"), { status: 401 });
    if (status === 503) throw error;
    throw Object.assign(new Error("无权管理更多玩法商品。"), { status: 403 });
  }
}
...
async function mutateLocal(mutator) {
  try {
    return await updateLocalProducts(mutator);
  } catch (error) {
    const msg = String(error?.message || error || "");
    if (/ENOENT|EROFS|read-only|EACCES|mkdir/i.test(msg)) {
      throw Object.assign(
        new Error("当前环境无法写入本地商品数据。请确认已执行 supabase/gameplay-products.sql 并配置 Supabase。"),
        { status: 503 }
      );
    }
    throw error;
...
async function listProducts() {
  if (hasDb()) {
    try {
      const rows = await supabaseJson(
        restUrl("gameplay_products", "?or=(deleted_at.is.null,status.neq.deleted)&order=sort_order.asc,updated_at.desc"),
      ...
      return {
        products: (Array.isArray(rows) ? rows : [])
          ...
          .map((item) => toPublicProduct(item, { admin: true })),
        source: "supabase",
      };
    } catch (error) {
  ...
  return {
    ...
    source: "local",
    message: hasDb()
      ? "gameplay_products 表未初始化，已使用只读本地商品数据。请执行 supabase/gameplay-products.sql。"
