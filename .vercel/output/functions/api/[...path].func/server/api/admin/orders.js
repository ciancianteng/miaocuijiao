...
function safeOrder(row, profiles, extras = {}) {
  ...
  const bossUid = String(boss.boss_uid || "").trim();
  const companionCode = String(
    companion.companion_code || companion.companionCode || companion.publicId || extras.companionCode || ""
  ).trim();
  const playerUid =
    (/^PW\d+$/i.test(companionCode) || /^P\d+$/i.test(companionCode) ? companionCode : "") || "";
  const isDevEmail = (v) => /\.meow\.test$/i.test(String(v || "")) || /^(service|boss|companion)\./i.test(String(v || ""));
  const orderNoRaw = String(row.order_no || row.orderNo || "").trim();
  const orderNo = /^MCJO\d+$/i.test(orderNoRaw) ? orderNoRaw.toUpperCase() : orderNoRaw && !/^[0-9a-f-]{36}$/i.test(orderNoRaw) ? orderNoRaw : "";
  return {
    id: row.id || row.uuid || "",
    orderNo,
    bossId: row.boss_id || "",
    bossUid: bossUid || "",
    bossName: boss.display_name || (!isDevEmail(boss.email) && boss.email) || bossUid || "-",
    playerName: companion.display_name || companion.nickname || (!isDevEmail(companion.email) && companion.email) || "待分配",
    companionName: companion.display_name || companion.nickname || "-",
    playerUid: playerUid || "",
    companionCode: playerUid || "",
    serviceStaff: service.display_name || (!isDevEmail(service.email) && service.email) || "-",
    serviceName: service.display_name || (!isDevEmail(service.email) && service.email) || "-",
    game: row.game || "",
...
function isTestOrder(order = {}) {
  if (order.is_test === true || order.isTest === true) return true;
  const blob = [order.order_no, order.title, order.description, order.note, order.order_type]
    .map((v) => String(v || ""))
    .join(" ");
  return /test|测试|MCJ_TEST|fake|demo|E2E|acceptance|\[TEST\]/i.test(blob);
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const [orders, profiles, companions] = await Promise.all([
        ...
        supabaseJson(restUrl("companion_profiles", "?select=user_id,companion_code,companion_uid,nickname&limit=2000"), {
          headers: serviceHeaders(),
        }).catch(() => []),
      ]);
      const map = (profiles || []).reduce((m, p) => {
      ...
      (companions || []).forEach((c) => {
        if (!c?.user_id) return;
        const prev = map[c.user_id] || { id: c.user_id };
        map[c.user_id] = {
          ...prev,
          companion_code: c.companion_code || prev.companion_code || "",
          companionCode: c.companion_code || prev.companionCode || "",
          nickname: c.nickname || prev.nickname || prev.display_name || "",
        };
      });
      ...
      const includeTest = String(req.query?.includeTest || "").trim() === "1";
      const sourceOrders = (orders || []).filter((o) => includeTest || !isTestOrder(o));
      const list = await Promise.all(
        sourceOrders.slice(0, 200).map(async (o) => {
          let grabCount = 0;
          let bossIntent = null;
          try {
