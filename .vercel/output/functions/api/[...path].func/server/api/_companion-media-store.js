...
export function assertImageUpload(decoded) {
  ...
  let normalized = mime === "image/jpg" ? "image/jpeg" : mime;
  // Mobile camera / some WebViews omit MIME or use octet-stream — sniff magic bytes.
  if (!ALLOWED_IMAGE_MIME.has(normalized) && !ALLOWED_IMAGE_MIME.has(mime)) {
    const sniffed = sniffImageMime(decoded.buffer);
    if (sniffed) normalized = sniffed;
  }
  if (!ALLOWED_IMAGE_MIME.has(normalized) && !ALLOWED_IMAGE_MIME.has(mime)) {
...
export function assertAudioUpload(decoded) {
  if (!decoded || !decoded.buffer) {
    throw Object.assign(new Error("文件格式无效，请选择语音文件"), { status: 400 });
  }
  const mime = String(decoded.contentType || "").toLowerCase();
  if (decoded.buffer.length > 20 * 1024 * 1024) {
    throw Object.assign(new Error("语音文件不能超过 20MB"), { status: 413 });
  }
  // Mobile browsers sometimes omit MIME; treat empty as mpeg so Storage accept still works.
  const finalMime = mime && ALLOWED_AUDIO_MIME.has(mime) ? mime : mime || "audio/mpeg";
  if (mime && !ALLOWED_AUDIO_MIME.has(mime) && !ALLOWED_AUDIO_MIME.has(finalMime)) {
    throw Object.assign(new Error("仅支持 mp3、wav、webm、ogg、m4a 语音格式"), { status: 400 });
  }
  return { ...decoded, contentType: finalMime || "audio/mpeg" };
}
