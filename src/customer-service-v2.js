...
(function(){
  ...
  function dashboardHtml(){
    ...
    return '<div class="cs-page-head"><div><h2>工作台</h2><p>主流程：接待会话、确认付款、推进订单。</p></div><div class="cs-actions"><button class="cs-btn primary" type="button" data-route="/customer-service/conversations">进入会话池</button><button class="cs-btn" type="button" data-route="/customer-service/orders">订单处理</button></div></div>'+
      ...
      metric('月薪标准',money(s.salaryRate!=null?s.salaryRate:((state.data&&state.data.workData&&state.data.workData.config)||{}).baseSalary||0))+
      metric('今日收入',money(s.incomeToday||0))+
      metric('本月收入',money(s.incomeMonth||0))+
      metric('累计收入',money(s.incomeTotal||0))+
      metric('可申请工资',money(s.withdrawableSalary!=null?s.withdrawableSalary:0))+
      metric('入职日期',esc(s.joinDate||((state.data&&state.data.workData&&state.data.workData.config)||{}).joinDate||'-'))+
      metric('下次入账日',esc(s.nextAccrualDate||((state.data&&state.data.payrollSummary)||{}).nextAccrualDate||'-'))+
  ...
  function conversationsHtml(){
    ...
    var orderPanel=order
      ?('<div class="cs-info-list">'+
        ...
        '<div><span>订单金额</span><strong>'+money(order.totalAmount)+'</strong></div>'+
        (order.platformFee!=null&&order.platformFee!==''?'<div><span>平台手续费</span><strong>'+money(order.platformFee)+(order.platformFeeRate!=null?' ('+esc(order.platformFeeRate)+'%)':'')+'</strong></div>':'')+
        (order.bossCommissionAmount!=null&&Number(order.bossCommissionAmount)>0?'<div><span>Boss 分成</span><strong>'+money(order.bossCommissionAmount)+(order.bossCommissionRate!=null?' ('+esc(order.bossCommissionRate)+'% · 平台手续费内)':'')+'</strong></div>':'')+
  ...
  function compensationHtml(){
    ...
    var allow=policy.allowCsApply!==false;
    var allowed=policy.allowedOrderStatuses||['in_progress','confirmed','completed'];
    var orders=((state.data&&state.data.orders)||[]).filter(function(o){return allowed.indexOf(o.status)>=0;});
    if(!allow){
      return '<div class="cs-page-head"><div><h2>申请补偿</h2><p>管理员已关闭客服补偿申请。</p></div></div><div class="cs-empty">当前不可申请补偿</div>';
    }
    return '<div class="cs-page-head"><div><h2>申请补偿</h2><p>仅进行中 / 已完成订单可申请；管理员审核通过后才会入账赠送猫粮。</p></div></div><form class="cs-card cs-form" data-compensation-form><label>老板 UID / 选择老板<select name="boss_id" required><option value="">请选择老板</option>'+bosses.map(function(b){return '<option value="'+esc(b.id)+'">'+esc(b.bossUid||b.uid||'')+' / '+esc(b.name)+'</option>'}).join('')+'</select></label><label>或输入老板 UID<input name="boss_uid" placeholder="MCJ00001"></label><label>关联订单<select name="related_order_id"><option value="">可选（建议选择）</option>'+orders.map(function(o){return '<option value="'+esc(o.id)+'">'+esc(o.orderNo)+' / '+esc(o.bossName)+' / '+esc(o.statusText||o.status)+'</option>'}).join('')+'</select></label><label>类型<select name="request_type"><option value="bad_review">差评安抚</option><option value="after_sale">售后补偿</option><option value="activity">活动奖励</option><option value="other">其他</option></select></label><label>建议补偿猫粮<input name="suggested_amount" type="number" min="1" required></label><label>差评或投诉原因<textarea name="reason" required></textarea></label><label>客服说明<textarea name="staff_note"></textarea></label><button class="cs-btn primary" type="submit">提交申请</button></form>';
  }
  function canApplyCompensation(o){
    var policy=(state.data&&state.data.compensationPolicy)||{};
    if(policy.allowCsApply===false)return false;
    var allowed=policy.allowedOrderStatuses||['in_progress','confirmed','completed'];
    return allowed.indexOf(String(o&&o.status||''))>=0;
  }
  function openOrderConv(orderId){
    var list=(state.data&&state.data.conversations)||[];
    var hit=list.find(function(c){return c.orderId===orderId;});
    if(hit){
      state.activeConversation=hit.id;
      state.convFilter=convBucket(hit);
      state.showConversationList=false;
      go('/customer-service/conversations');
      loadActiveConversationMessages(hit.id);
      return;
    }
    toast('未找到关联会话，请到会话池查看');
    go('/customer-service/conversations');
  }
  function openOrderDetailModal(o, mode){
    mode=mode||'detail';
    ...
    var rows=[
      ...
      ['陪玩',o.companionName||'-'],
      ['游戏',o.game||'-'],
      ['订单金额',money(o.totalAmount)],
      ['平台手续费',o.platformFee!=null?money(o.platformFee)+(o.platformFeeRate!=null?' ('+o.platformFeeRate+'%)':''):'-'],
      ['Boss 分成',o.bossCommissionAmount!=null?money(o.bossCommissionAmount)+(o.bossCommissionRate!=null?' · '+o.bossCommissionRate+'%':''):'-'],
    ...
    if(mode==='refund'){
      rows.push(['退款状态',o.statusText||o.status||'-']);
      rows.push(['退款说明',o.refundNote||o.note||'-']);
    }
    if(mode==='pay'){
  ...
  function orderRow(o){
    ...
    if(st==='awaiting_payment'){
      ...
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="pay">查看付款记录</button>');
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="review">查看评价</button>');
      if(st==='completed'&&canApplyCompensation(o)&&actions.length<4){
        /* keep max 4; compensation already covered when needed via dedicated page */
      }
    }else if(st==='cancelled'){
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="detail">查看详情</button>');
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="cancel">查看取消原因</button>');
    }else if(st==='refunded'){
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="refund">查看退款详情</button>');
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="pay">查看退款凭证</button>');
    }else if(st==='refund_requested'){
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="refund">查看售后</button>');
      actions.push('<button class="cs-btn danger" data-refund-order="'+esc(o.id)+'">处理退款</button>');
    }else{
      actions.push('<button class="cs-btn ghost" data-order-detail="'+esc(o.id)+'" data-detail-mode="detail">查看详情</button>');
    }

    // Hard cap: at most 4 action buttons
  ...
  function reportsHtml(){var work=(state.data&&state.data.workData)||{},salary=work.salary||{},cur=salary.current||{},history=salary.history||[],attRows=(work.attendance&&work.attendance.rows)||[],notices=(state.data&&state.data.notifications)||[],payrolls=(state.data&&state.data.payrolls)||[],settlements=(state.data&&state.data.commissionSettlements)||work.commissionSettlements||[],sum=(state.data&&state.data.summary)||{},cfg=work.config||{},ps=(state.data&&state.data.payrollSummary)||{},withdrawable=sum.withdrawableSalary!=null?sum.withdrawableSalary:0,salaryRate=sum.salaryRate!=null?sum.salaryRate:(cfg.baseSalary||cur.baseSalary||0),joinDate=sum.joinDate||cfg.joinDate||ps.joinDate||'-',periodLabel=(sum.salaryPeriodStart||cur.periodStart||'')+' ~ '+(sum.salaryPeriodEnd||cur.periodEnd||''),nextAccrual=sum.nextAccrualDate||ps.nextAccrualDate||'-',eligible=sum.eligibleSalaryPeriod||ps.eligibleSalaryPeriod||'';var noticeBlock=notices.length?'<section class="cs-card" style="margin-bottom:14px"><h3 style="margin:0 0 8px">工资通知</h3>'+notices.slice(0,8).map(function(n){return '<div style="padding:8px 0;border-bottom:1px solid #eee"><strong>'+esc(n.title||'通知')+'</strong><div style="color:#9ca3af;font-size:12px;margin-top:4px">'+esc(n.body||'')+'</div><div style="color:#6b7280;font-size:11px;margin-top:4px">'+esc(n.at||'')+'</div></div>'}).join('')+'</section>':'';var withdrawBlock='<section class="cs-card" style="margin-bottom:14px"><h3 style="margin:0 0 8px">月薪结算</h3><p style="margin:0 0 10px;color:#6b7280;font-size:13px">月薪标准是费率，不是立即可提余额。新入职可申请为 0；周期结束后才生成一笔薪资结算。周五统一发放。</p><div class="cs-info-list"><div><span>入职日期</span><strong>'+esc(joinDate)+'</strong></div><div><span>月薪标准</span><strong>'+money(salaryRate)+'</strong></div><div><span>本薪资周期</span><strong>'+esc(periodLabel||'-')+'</strong></div><div><span>下次入账日期</span><strong>'+esc(nextAccrual)+'</strong></div><div><span>可申请周期</span><strong>'+esc(eligible||'（当前周期未结束）')+'</strong></div><div><span>可申请金额</span><strong>'+money(withdrawable)+'</strong></div><div><span>已申请金额</span><strong>'+money(ps.appliedAmount||0)+'</strong></div><div><span>待周五发放</span><strong>'+money(ps.pendingFridayAmount||0)+'</strong></div><div><span>预计发放日期</span><strong>'+esc((((state.data&&state.data.weeklySettlement)||{}).nextSettlementDate||ps.nextSettlementDate||'-'))+( ((state.data&&state.data.weeklySettlement)||{}).nextSettlementDate?'（星期五）':'')+'</strong></div></div><div class="cs-actions" style="margin-top:12px"><button class="cs-btn primary" type="button" data-request-salary-withdraw '+(Number(withdrawable)<=0?'disabled':'')+'>申请已结束周期结算（'+money(withdrawable)+'）</button></div></section>';var rewardBlock='<section class="cs-table-wrap" style="margin-top:14px"><h3 style="margin:0 0 10px">奖励记录</h3><table class="cs-table"><thead><tr><th>订单号</th><th>奖励类型</th><th>固定奖励</th><th>提成</th><th>夜班补贴</th><th>全勤奖励</th><th>退款扣回</th><th>最终金额</th><th>状态</th></tr></thead><tbody>'+(settlements.length?settlements.map(function(r){return '<tr><td>'+esc(r.orderNo||'-')+'</td><td>'+esc(r.rewardType||'order_commission')+'</td><td>'+money(r.fixedRewardRm||0)+'</td><td>'+money(r.percentCommissionRm||0)+'</td><td>'+money(r.nightShiftRm||0)+'</td><td>'+money(r.attendanceBonusRm||0)+'</td><td>'+money(r.clawbackRm||0)+'</td><td>'+money(r.finalAmountRm||0)+'</td><td>'+esc(r.status||'-')+'</td></tr>'}).join(''):'<tr><td colspan="9">暂无奖励记录（完成订单后按后台佣金设置自动入账）</td></tr>')+'</tbody></table></section>';var payrollBlock='<section class="cs-table-wrap" style="margin-top:14px"><h3 style="margin:0 0 10px">历史结算记录</h3><table class="cs-table"><thead><tr><th>单号</th><th>周期</th><th>底薪</th><th>奖金</th><th>扣款</th><th>应发</th><th>预计发放</th><th>状态</th><th></th></tr></thead><tbody>'+(payrolls.length?payrolls.map(function(p){return '<tr><td>'+esc(p.payrollNo||'-')+'</td><td>'+esc((p.periodStart||'')+' ~ '+(p.periodEnd||''))+'</td><td>'+money(p.baseSalaryRm||0)+'</td><td>'+money(p.bonusRm||0)+'</td><td>'+money(p.deductionRm||0)+'</td><td>'+money(p.netSalaryRm||0)+'</td><td>'+esc(p.settlementDate||'-')+'</td><td>'+esc(p.statusText||p.status||'-')+'</td><td>'+(p.status!=='completed'?'<button class="cs-btn ghost" type="button" data-payroll-appeal="'+esc(p.id)+'">申诉</button>':(p.hasReceipt||p.receiptUrl?'<a class="cs-btn ghost" href="'+esc(p.receiptUrl||'#')+'" target="_blank" rel="noopener">收据</a>':'-'))+'</td></tr>'}).join(''):'<tr><td colspan="9">暂无发放记录</td></tr>')+'</tbody></table></section>';return '<div class="cs-page-head"><div><h2>工资中心</h2><p>月薪标准按费率展示；周期结束后才可申请一笔结算。订单提成按结算快照入账，改配置不影响历史单。</p></div></div>'+noticeBlock+withdrawBlock+'<section class="cs-grid cs-metrics">'+metric('月薪标准',money(salaryRate))+metric('全勤奖励',money(cur.attendanceBonus||0))+metric('接待奖励',money(cur.receptionBonus||0))+metric('订单提成',money(cur.orderCommission||0))+metric('夜班补贴',money(cur.nightShiftAllowance||0))+metric('迟到扣款',money(cur.lateDeduction||0))+metric('缺勤扣款',money(cur.absenceDeduction||0))+metric('其他调整',money(cur.otherAdjustment||0))+metric('本月预计工资',money(cur.totalSalary||0))+metric('工资状态',cur.status||'统计中')+'</section><section class="cs-table-wrap" style="margin-top:14px"><h3 style="margin:0 0 10px">本月打卡（工资计算依据）</h3><table class="cs-table"><thead><tr><th>日期</th><th>上班</th><th>下班</th><th>工时</th><th>迟到</th><th>缺勤</th><th>状态</th></tr></thead><tbody>'+(attRows.length?attRows.map(function(r){return '<tr><td>'+esc(r.reportDate||r.date||'-')+'</td><td>'+esc(r.clockInText||'-')+'</td><td>'+esc(r.clockOutText||'-')+'</td><td>'+esc(r.workHours!=null?r.workHours:'-')+'</td><td>'+esc(r.isLate?'是':'否')+'</td><td>'+esc(r.isAbsent?'是':'否')+'</td><td>'+esc(r.attendanceStatus||'-')+'</td></tr>'}).join(''):'<tr><td colspan="7">暂无打卡记录</td></tr>')+'</tbody></table></section>'+rewardBlock+payrollBlock+'<section class="cs-table-wrap" style="margin-top:14px"><table class="cs-table"><thead><tr><th>月份</th><th>月薪标准</th><th>全勤奖励</th><th>接待奖励</th><th>订单提成</th><th>扣款合计</th><th>预计工资</th><th>状态</th></tr></thead><tbody>'+(history.length?history.map(function(r){var deductions=(Number(r.lateDeduction||0)+Number(r.absenceDeduction||0)+Number(r.earlyLeaveDeduction||0));return '<tr><td>'+esc(r.salaryMonth||'-')+'</td><td>'+money(r.baseSalary||0)+'</td><td>'+money(r.attendanceBonus||0)+'</td><td>'+money(r.receptionBonus||0)+'</td><td>'+money(r.orderCommission||0)+'</td><td>'+money(deductions)+'</td><td>'+money(r.totalSalary||0)+'</td><td>'+esc(r.status||'统计中')+'</td></tr>'}).join(''):'<tr><td colspan="8">暂无工资记录</td></tr>')+'</tbody></table></section>'}
  function reportStatus(s){return ({pending:'待审核',approved:'已批准',rejected:'已拒绝',paid:'已支付',completed:'已发放'})[s]||s||'-'}
  function profileHtml(){var s=(state.data&&state.data.staff)||state.session.user||{},work=(state.data&&state.data.workData)||{},cfg=work.config||{},att=work.attendance||{},sum=(state.data&&state.data.summary)||{},ps=(state.data&&state.data.payrollSummary)||{},csName=String(s.name||s.displayName||'').trim()||'客服',avatar='<div class="cs-avatar" style="width:64px;height:64px;display:grid;place-items:center">'+esc(csName.slice(0,1))+'</div>';return '<section class="cs-card"><h2>我的资料</h2><div class="cs-user-card">'+avatar+'<div><strong>'+esc(csName)+'</strong><div style="color:#9ca3af;margin-top:4px">'+esc(cfg.shiftName||'默认班次')+'</div></div></div><div class="cs-info-list"><div><span>当前班次</span><strong>'+esc((cfg.shiftStart||'09:00')+' - '+(cfg.shiftEnd||'18:00'))+'</strong></div><div><span>入职日期</span><strong>'+esc(sum.joinDate||cfg.joinDate||ps.joinDate||'-')+'</strong></div><div><span>月薪标准</span><strong>'+money(sum.salaryRate!=null?sum.salaryRate:cfg.baseSalary||0)+'</strong></div><div><span>本薪资周期</span><strong>'+esc((sum.salaryPeriodStart||'')+' ~ '+(sum.salaryPeriodEnd||'')||'-')+'</strong></div><div><span>下次入账日期</span><strong>'+esc(sum.nextAccrualDate||ps.nextAccrualDate||'-')+'</strong></div><div><span>可申请工资</span><strong>'+money(sum.withdrawableSalary!=null?sum.withdrawableSalary:0)+'</strong></div><div><span>在线状态</span><strong>'+esc(sum.currentReceptions>0?'接待中':'在线')+'</strong></div><div><span>今日打卡状态</span><strong>'+esc((work.todayAttendance&&work.todayAttendance.attendanceStatus)||'未打卡')+'</strong></div><div><span>本月出勤</span><strong>'+esc((att.actualDays||0)+' / '+(att.standardDays||0))+'</strong></div><div><span>本月预计工资</span><strong>'+money(sum.estimatedSalary||0)+'</strong></div><div><span>历史工资记录</span><strong>'+esc((work.salary&&work.salary.history&&work.salary.history.length)||0)+' 条</strong></div></div></section><section class="cs-card" style="margin-top:14px"><h3 style="margin:0 0 10px">消息通知</h3><div id="mcjWebPushSettingsMount" class="mcj-webpush-cs-mount"></div><p style="margin:10px 0 0;color:#9ca3af;font-size:12px;line-height:1.45">开启后可及时收到订单与会话相关推送；关闭开关仅取消本机订阅。</p></section>'}
  ...
  var wdBtn=e.target.closest('[data-request-salary-withdraw]');if(wdBtn){if(wdBtn.disabled)return;if(!confirm('确认申请已结束薪资周期的结算？金额由系统计算，不可修改，将进入待周五结算。'))return;wdBtn.disabled=true;api('request_salary_withdraw',{}).then(function(res){toast(res.message||('已申请 '+money(res.amount||0)+'，预计发放 '+(res.settlementDate||'')));return softRefresh()}).catch(function(err){toast(err.message||'申请失败');wdBtn.disabled=false});return}
  ...
  var c=e.target.closest('[data-conversation]');if(c){e.preventDefault();e.stopPropagation();var cid=c.dataset.conversation;setTimeout(function(){
    ...
    if(urge){
      e.preventDefault();
      urge.disabled=true;
      api('urge_companion',{id:urge.dataset.urgeCompanion}).then(function(res){toast(res.message||'已催单');}).catch(function(err){toast(err.message||'催单失败');}).then(function(){urge.disabled=false;});
      return;
    }
    var retHall=e.target.closest('[data-return-grab-hall]');
    if(retHall){
      e.preventDefault();
      if(!confirm('确认将该订单返回抢单大厅？指定陪玩将被解除。'))return;
      retHall.disabled=true;
      api('return_to_grab_hall',{id:retHall.dataset.returnGrabHall}).then(function(res){toast(res.message||'已返回抢单大厅');return softRefresh();}).catch(function(err){retHall.disabled=false;toast(err.message||'操作失败');});
      return;
    }
    var od=e.target.closest('[data-order-detail]');
    if(od){
      e.preventDefault();
      var oid=od.dataset.orderDetail;
      var mode=od.getAttribute('data-detail-mode')||'detail';
      var order=((state.data&&state.data.orders)||[]).find(function(x){return x.id===oid;})||{};
      openOrderDetailModal(order,mode);
      return;
    }
    var openChat=e.target.closest('[data-open-order-chat]');
    if(openChat){
      e.preventDefault();
      openOrderConv(openChat.dataset.openOrderChat);
      return;
    }
  ...
  function openAssign(id, opts){
    ...
    function grabCard(g){
      ...
      return '<article class="cs-grab-card" style="border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:12px;margin:8px 0">'+
        ...
        '<button class="cs-btn primary" type="button" data-confirm-assign="'+esc(id)+'" data-companion-id="'+esc(g.companionId||c.id||'')+'">'+(preferred?'确认指定（老板意向）':'确认指定')+'</button>'+
    ...
    var grabBlock=grabs.length
      ?('<div><h4 style="margin:0 0 8px">已抢单陪玩（'+grabs.length+'）</h4><p class="cs-note">公开抢单订单只能从抢单人中确认指定；确认后订单离开抢单大厅，进入待陪玩确认。</p>'+grabs.map(grabCard).join('')+'</div>')
      :'<p class="cs-composer-hint">暂无抢单记录。请等待陪玩抢单；老板提交「我要她」意向后，再点击确认指定。</p>';
    var isPublicHall=['pending','waiting_boss_confirm'].indexOf(String(order.status||''))>=0 && !order.companionId;
    var freeAssign=isPublicHall
      ? '<p class="cs-composer-hint">公开抢单路径不支持直接指定任意陪玩。请从上方抢单人中确认指定，或走 VIP 指定下单。</p>'
      : ('<hr style="border:0;border-top:1px solid rgba(255,255,255,.1);margin:14px 0">'+
        '<label>其他陪玩（非抢单大厅路径）<select data-assign-companion>'+((state.data&&state.data.companions)||[]).map(function(p){return '<option value="'+esc(p.id)+'">'+esc(p.name)+(p.companionCode||p.publicId?(' / '+(p.companionCode||p.publicId)):'')+' / '+esc(p.game||'-')+'</option>'}).join('')+'</select></label>'+
    ...
    modal('<div class="cs-dialog-head"><h3>'+(isPublicHall?'确认指定':'指定陪玩')+'</h3><button class="cs-btn" type="button" data-close-modal>关闭</button></div>'+
      (intent?'<p class="cs-composer-hint">老板意向：'+esc(intent.companionName||intent.companionId)+'（请客服确认指定）</p>':'')+
      grabBlock+
      freeAssign);
