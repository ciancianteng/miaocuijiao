(function(){
  ...
  function navLabel(key, fb){ return tt(key, fb); }
  ...
  function buildNav(){
    return [
      ['dashboard',navLabel('companion.nav.dashboard','工作台'),'/companion/dashboard'],
      ['hall',navLabel('companion.nav.hall','抢单大厅'),'/companion/order-hall'],
      ['orders',navLabel('companion.nav.orders','我的订单'),'/companion/orders'],
      ['earnings',navLabel('companion.nav.earnings','收益中心'),'/companion/earnings'],
      ['profile',navLabel('companion.nav.profile','我的资料（公开）'),'/companion/profile'],
      ['account',navLabel('companion.nav.account','账号中心（隐私）'),'/companion/account'],
      ['messages',navLabel('companion.nav.messages','消息中心'),'/companion/messages'],
      ['settings',navLabel('companion.nav.settings','设置'),'/companion/settings']
    ];
  }
  function buildBottomNav(){
    return [
      ['dashboard',navLabel('companion.bottom.dashboard','工作台'),'/companion/dashboard'],
      ['hall',navLabel('companion.bottom.hall','抢单'),'/companion/order-hall'],
      ['orders',navLabel('companion.bottom.orders','订单'),'/companion/orders'],
      ['earnings',navLabel('companion.bottom.earnings','收益'),'/companion/earnings'],
      ['account',navLabel('companion.bottom.account','账号'),'/companion/account']
    ];
  }
  var NAV=buildNav();
  var BOTTOM_NAV=buildBottomNav();
  var ISOLATION_NAV=[
  ...
  window.addEventListener('mcj:localechange',function(){NAV=buildNav();BOTTOM_NAV=buildBottomNav();try{if(typeof paint==='function')paint();else if(typeof render==='function')render();}catch(e){}});
  ...
  function title(){return ({dashboard:tt('companion.nav.dashboard','工作台'),hall:tt('companion.nav.hall','抢单大厅'),orders:tt('companion.nav.orders','我的订单'),earnings:tt('companion.nav.earnings','收益中心'),wallet:tt('companion.nav.earnings','收益中心'),profile:isIsolationMode()?'申请资料':'编辑公开资料',account:isIsolationMode()?'账号资料':tt('companion.nav.account','账号中心（隐私）'),mine:tt('companion.nav.account','账号中心（隐私）'),withdraw:'提现',messages:tt('companion.nav.messages','消息中心'),settings:tt('companion.nav.settings','设置'),popularity:'我的人气',rules:'陪玩规则','review-status':'审核状态'})[state.route]||tt('companion.title','陪玩端')}
  ...
  function pwGalleryItemHtml(item,idx,total){
    ...
    var statusHtml=uploading?'<span class="pw-media-uploading">上传中…</span>':(failed?'<span class="pw-media-uploading is-fail">上传失败，请重试</span>':'');
    var delAttr=pendingId
      ? 'data-gallery-dismiss-pending="'+esc(pendingId)+'"'
      : 'data-delete-media="'+esc(item.id||'')+'"';
    var canReorder=!pendingId&&!uploading&&!failed;
    return '<article class="pw-media-thumb'+(uploading?' is-uploading':'')+(failed?' is-fail':'')+'" data-media-id="'+esc(item.id||pendingId||'')+'"'+(pendingId?' data-gallery-pending="'+esc(pendingId)+'"':'')+'>'+
      '<button type="button" class="pw-media-thumb-hit" '+(uploading||failed?'disabled':' data-gallery-preview="'+esc(url)+'"')+' aria-label="预览相册照片">'+
      '<img src="'+esc(url)+'" alt="相册照片">'+statusHtml+
      '</button>'+
  ...
  function pickCompanionGallery(){
    ...
    function onFiles(files){uploadGalleryFiles(files||[])}
    triggerPwHiddenPick('image/*',false,function(file){if(file)onFiles([file])},{multiple:true,onFiles:onFiles});
  }
  ...
  document.addEventListener('click',function(e){
    ...
    var pickCam=e.target.closest('[data-pw-pick-gallery-camera]');
    if(pickCam){
      e.preventDefault();
      e.stopPropagation();
      if(!pickCam.disabled)pickCompanionGalleryCamera();
      return;
    }
    var dismissPending=e.target.closest('[data-gallery-dismiss-pending]');
    if(dismissPending){
      e.preventDefault();
      var pid=dismissPending.getAttribute('data-gallery-dismiss-pending')||'';
      state.galleryPending=(state.galleryPending||[]).filter(function(p){return !p||p._localId!==pid});
      if(!(state.galleryPending||[]).some(function(p){return p&&p._uploading}))state.uploadBusy='';
      paint({preserveScroll:true});
      return;
    }
    // Legacy button (if any) — keep multi pick.
    ...
    if(input.hasAttribute('data-upload-gallery')){
      // Legacy gallery tile embeds multiple input — let native label/input handle multi-select.
      return;
    }
    e.preventDefault();
    e.stopPropagation();
    openPwUploadSourceSheet(function(opts){
      ...
      triggerPwHiddenPick(accept,!!opts.capture,function(file){
        if(input.hasAttribute('data-upload-doc'))uploadPrivateDoc(input.getAttribute('data-upload-doc')||'',file);
      },{multiple:false});
    });
  },true);
  ...
  document.addEventListener('change',function(e){
    ...
    var galleryMulti=e.target.closest('[data-pw-gallery-multi]');
    if(galleryMulti&&galleryMulti.files&&galleryMulti.files.length){
