(function(){
  ...
  document.addEventListener('click',function(e){
    ...
    if(sendRegOtp){
      ...
      try{
        var until=Number(sessionStorage.getItem('mcj_otp_cd:send_register_otp:companion:'+email)||0);
        var leftStored=Math.max(0,Math.ceil((until-Date.now())/1000));
        if(leftStored>0){
          state.registerBusy=false;
          state.registerCooldownUntil=Date.now()+leftStored*1000;
          state.loginError='发送过于频繁，请 '+leftStored+' 秒后再试。';
          paint();
          return;
        }
      }catch(e){}
      fetch('/api/auth',{method:'POST',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({action:'send_register_otp',email:email,role:'companion'})})
        ...
        .then(function(j){
          ...
          var sec=Number(j.retryAfterSec)||60;
          state.registerCooldownUntil=Date.now()+sec*1000;
