(function(){
  ...
  function playerDetailHtml(item,mode,focus){
    ...
    var account=playerDetailRows([['当前在线状态',item.online],['账号状态',item.account],['是否已设置密码',(item.raw&&(item.raw.hasPassword||item.raw.has_password))?'是':'否'],['注册邮箱',item.email||'-'],['最近密码重置',(item.raw&&(item.raw.passwordSetAt||item.raw.password_set_at))||'-']])+(edit?'<div class="player-edit-grid">'+playerSelectField('账号状态','accountStatus',playerAccountOptions(item.account))+'</div>':'')+'<div class="player-edit-grid" style="margin-top:10px"><button class="mini-btn" type="button" data-player-sec="send_password_reset" data-player-id="'+esc(item.id)+'">发送密码重置邮件</button><button class="mini-btn" type="button" data-player-sec="force_change_password" data-player-id="'+esc(item.id)+'">强制下次改密</button><button class="mini-btn" type="button" data-player-sec="revoke_sessions" data-player-id="'+esc(item.id)+'">注销全部会话</button><button class="mini-btn" type="button" data-player-sec="enable" data-player-id="'+esc(item.id)+'">解封账号</button></div><div class="admin-sync-note">后台不可查看真实密码或哈希；重置仅发送邮箱验证码由用户自行设置。</div>';
