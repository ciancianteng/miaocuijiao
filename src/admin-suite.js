(function(){
  ...
  function orderManagementHtml(orders,summary,configured){
    ...
    return '<div class="order-admin">'+
      ...
      (!orders.length?'<div class="order-empty"><strong>暂无订单</strong><span>当前没有订单记录。可通过「客服创建订单」录入真实订单。</span>'+(isLocalAdminPreview()?'<button class="btn" data-order-dev-test type="button">创建测试订单（仅本地）</button>':'')+'<button class="btn primary" data-order-create-service type="button">客服创建订单</button></div>':'')+
  ...
  function playerIncomeRows(item){var rows=playerArrayValue(item.raw.incomeRows||item.raw.income_records||item.raw.walletTransactions).map(function(row){var st=row.statusDetail||row.status||'-';return '<tr><td>'+esc(row.orderNo||row.order_no||row.id||'-')+'</td><td>'+esc(row.type||'-')+'</td><td>'+esc(row.amount||'-')+'</td><td>'+esc(st)+'</td><td>'+esc(row.time||row.created_at||row.createdAt||'-')+'</td></tr>'});return rows;}
  function playerDetailHtml(item,mode,focus){
    ...
    var income=playerDetailRows([['完成订单数',item.totalOrders],['退款订单',item.refundOrders],['总收入',item.totalIncome],['平台抽成',item.platformShare],['可提现余额',item.withdrawable],['已提现金额',item.withdrawn]])+playerRecordTable(['订单号','服务','金额','状态','时间'],playerOrdersRows(item),'暂无真实历史订单')+playerRecordTable(['订单号','类型','金额','提现状态','时间'],playerIncomeRows(item),'暂无真实收入流水');
