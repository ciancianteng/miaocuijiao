(function(){
  ...
  function renderOrderDetail(order){
    ...
    var detail=[
      ...
      ['订单评价',reviewLine],
      ['老板支付金额',moneyText(orderValue(order,['originalAmount','original_amount','amount'],order.amount))],['优惠金额',moneyText(orderValue(order,['discountAmount','discount_amount'],0))],['实际支付金额',moneyText(order.amount)],['陪玩佣金比例',orderValue(order,['playerCommissionRate','player_commission_rate'],'-')],['陪玩应得收入',moneyText(orderValue(order,['companionIncome','playerIncome','companion_income'],order.playerIncome))],['平台抽成',moneyText(orderValue(order,['platformFee','platform_fee'],order.amount-order.playerIncome))],['老板分成(平台抽成内)',moneyText(orderValue(order,['bossCommissionAmount','boss_commission_amount'],0))],['老板分成比例',orderValue(order,['bossCommissionRate','boss_commission_rate'],'-')+'% · '+orderValue(order,['bossCommissionRateSource','boss_commission_rate_source'],'')],['结算等级快照',orderValue(order,['bossLevelCode','boss_level_code'],'-')],['直属返点',moneyText(orderValue(order,['directRebate','direct_rebate'],0))],['退款金额',moneyText(orderValue(order,['refundAmount','refund_amount'],0))],['最终平台利润',moneyText(order.platformProfit)],['结算状态',orderValue(order,['settlementStatus','settlement_status'],'待结算')]
    ];
    ...
    var feeSnap=orderValue(order,['platformFee','platform_fee'],order.amount-order.playerIncome);
    var bossSnap=orderValue(order,['bossCommissionAmount','boss_commission_amount'],0);
    var compSnap=orderValue(order,['companionIncome','playerIncome','companion_income'],order.playerIncome);
    var amountSnap=orderValue(order,['originalAmount','original_amount','amount'],order.amount);
    var moneySplit='<div class="bcr-money-split" style="margin:12px 0 14px"><div class="step"><span>订单金额</span><strong>'+esc(moneyText(amountSnap))+'</strong></div><div class="step fee"><span>平台抽成</span><strong>'+esc(moneyText(feeSnap))+'</strong></div><div class="step boss"><span>Boss 分成</span><strong>'+esc(moneyText(bossSnap))+'</strong></div><div class="step comp"><span>陪玩收入</span><strong>'+esc(moneyText(compSnap))+'</strong></div></div><p class="admin-sync-note" style="margin:0 0 12px">Boss 分成从平台抽成支付，不扣陪玩收入。快照不因后续等级变更改写。</p>';
    body.innerHTML='<h2>订单详情</h2><p class="muted">'+esc(order.id)+' · '+esc(order.type)+' · '+esc(order.orderStatus)+'</p><div class="order-detail-tabs">'+tabs.map(function(tab){return '<span>'+esc(tab)+'</span>'}).join('')+'</div>'+timing+moneySplit+'<div class="detail-list">'+detail.map(function(item){return '<div><span>'+esc(item[0])+'</span><strong>'+esc(item[1])+'</strong></div>'}).join('')+'</div><div class="admin-sync-note">聊天记录、售后记录、支付流水和操作日志均应通过订单 ID 关联统一数据库；当前详情页不会生成模拟上下文。</div>';
    openAdminModal();
  }
  ...
  var sectionTitles={
    ...
    'points-settings':['积分设置','Boss 订单积分：实付金额 × 倍率（与人气榜独立）'],
    'boss-companion-relations':['直属关系管理','Boss ↔ Companion 运营直属绑定 / 换绑 / 解绑 / 历史（reason 审计）'],
    'boss-levels':['Boss 等级','可配置门槛与分成%；手动钉选永久/到期；历史结算快照不变'],
