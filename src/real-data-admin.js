(function () {
  ...
  async function saveCompanion() {
    ...
    next.companions.unshift({ id: "player_" + Date.now(), name: data.name, game: data.game, level: data.level, rating: data.rating, price: data.price, sort: Number(data.sort) || 1, auditStatus: data.auditStatus, visible: data.visible === "true", buttonStyle: data.buttonStyle, avatar: avatar, cover: cover, tags: data.tags, bio: data.bio });
    save(next); f.reset();
  }
  function saveRange() {
  ...
  function renderTables() {
    ...
    if (cr) cr.innerHTML = (d.companions || []).map(function (p) { return '<tr><td>' + (p.cover ? '<img src="' + p.cover + '">' : "-") + '</td><td>' + esc(p.name) + '</td><td>' + esc(p.game) + '</td><td>' + esc(p.price) + '</td><td>' + esc(p.auditStatus) + '</td><td><button class="mcj-real-btn" data-del="companions" data-id="' + esc(p.id) + '">删除</button></td></tr>'; }).join("") || '<tr><td colspan="6">暂无陪玩资料，请创建并审核通过。</td></tr>';
