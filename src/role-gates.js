(function () {
  ...
  function afterAuthSuccess(result, options) {
    ...
    var go = function () {
      ...
      // Homepage「申请成为陪玩」resume: always leave current page for apply.
      if (applyAfterLogin) {
  ...
  // companion-apply entry: open login first; login success → /companion-apply.html (do not touch auth gate).
    ...
    function (event) {
      ...
      if (wantsApply) {
        ...
        if (onHome && window.MCJModal && typeof window.MCJModal.openLogin === "function") {
          ...
          // Re-assert after openLogin in case modal/boot wiped session keys.
          rememberCompanionApplyAfterLogin();
          return;
        }
